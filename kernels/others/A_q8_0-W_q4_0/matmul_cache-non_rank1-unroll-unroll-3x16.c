    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    ...
    {
        __m256 sum_packed_fp_0_j0 = _mm256_setzero_ps();
        __m256 sum_packed_fp_1_j0 = _mm256_setzero_ps();
        __m256 sum_packed_fp_2_j0 = _mm256_setzero_ps();
        __m256 sum_packed_fp_0_j1 = _mm256_setzero_ps();
        __m256 sum_packed_fp_1_j1 = _mm256_setzero_ps();
        __m256 sum_packed_fp_2_j1 = _mm256_setzero_ps();
        
        for (int p = 0; p < block_k; p += Q_BLK_SIZE)
        {
            int block_idx = p / Q_BLK_SIZE;
            
            // Load scaling factors for j and j+1
            float sB_j0 = *blockSB(block_idx, j, block_k/Q_BLK_SIZE);
            float sB_j1 = *blockSB(block_idx, j+1, block_k/Q_BLK_SIZE);
            ...
            float fused_s0_j0 = sA0 * sB_j0;
            float fused_s1_j0 = sA1 * sB_j0;
            float fused_s2_j0 = sA2 * sB_j0;
            float fused_s0_j1 = sA0 * sB_j1;
            float fused_s1_j1 = sA1 * sB_j1;
            float fused_s2_j1 = sA2 * sB_j1;
            ...
            // Load B blocks for j and j+1
            __m256i b_pack_int8_j0 = bytes_from_nibbles_32(blockB(p/2, j, block_k/2));
            __m256i b_pack_int8_j1 = bytes_from_nibbles_32(blockB(p/2, j+1, block_k/2));
            
            __m256i offset = _mm256_set1_epi8(8);
            b_pack_int8_j0 = _mm256_sub_epi8(b_pack_int8_j0, offset);
            b_pack_int8_j1 = _mm256_sub_epi8(b_pack_int8_j1, offset);
            
            // Compute for j
            __m256 tmp0_j0 = mul_sum_i8_pairs_float(a_pack_int8_0, b_pack_int8_j0);
            __m256 tmp1_j0 = mul_sum_i8_pairs_float(a_pack_int8_1, b_pack_int8_j0);
            __m256 tmp2_j0 = mul_sum_i8_pairs_float(a_pack_int8_2, b_pack_int8_j0);
            
            // Compute for j+1
            __m256 tmp0_j1 = mul_sum_i8_pairs_float(a_pack_int8_0, b_pack_int8_j1);
            __m256 tmp1_j1 = mul_sum_i8_pairs_float(a_pack_int8_1, b_pack_int8_j1);
            __m256 tmp2_j1 = mul_sum_i8_pairs_float(a_pack_int8_2, b_pack_int8_j1);
            
            // Accumulate for j
            sum_packed_fp_0_j0 = _mm256_fmadd_ps(tmp0_j0, _mm256_set1_ps(fused_s0_j0), sum_packed_fp_0_j0);
            sum_packed_fp_1_j0 = _mm256_fmadd_ps(tmp1_j0, _mm256_set1_ps(fused_s1_j0), sum_packed_fp_1_j0);
            sum_packed_fp_2_j0 = _mm256_fmadd_ps(tmp2_j0, _mm256_set1_ps(fused_s2_j0), sum_packed_fp_2_j0);
            
            // Accumulate for j+1
            sum_packed_fp_0_j1 = _mm256_fmadd_ps(tmp0_j1, _mm256_set1_ps(fused_s0_j1), sum_packed_fp_0_j1);
            sum_packed_fp_1_j1 = _mm256_fmadd_ps(tmp1_j1, _mm256_set1_ps(fused_s1_j1), sum_packed_fp_1_j1);
            sum_packed_fp_2_j1 = _mm256_fmadd_ps(tmp2_j1, _mm256_set1_ps(fused_s2_j1), sum_packed_fp_2_j1);
        }

        // Handle the case where block_n is not a multiple of 2
        bool process_j0 = true;
        ...
        if (process_j0) {
            ...
            {
                case 3:
                    *C(0, j, NDIM) += hsum_float_8(sum_packed_fp_0_j0);
                    *C(1, j, NDIM) += hsum_float_8(sum_packed_fp_1_j0);
                    *C(2, j, NDIM) += hsum_float_8(sum_packed_fp_2_j0);
                    break;
                case 2:
                    *C(0, j, NDIM) += hsum_float_8(sum_packed_fp_0_j0);
                    *C(1, j, NDIM) += hsum_float_8(sum_packed_fp_1_j0);
        ...
        if (process_j1) {
            ...
            {
                case 3:
                    *C(0, j+1, NDIM) += hsum_float_8(sum_packed_fp_0_j1);
                    *C(1, j+1, NDIM) += hsum_float_8(sum_packed_fp_1_j1);
                    *C(2, j+1, NDIM) += hsum_float_8(sum_packed_fp_2_j1);
                    break;
                case 2:
                    *C(0, j+1, NDIM) += hsum_float_8(sum_packed_fp_0_j1);
                    *C(1, j+1, NDIM) += hsum_float_8(sum_packed_fp_1_j1);
