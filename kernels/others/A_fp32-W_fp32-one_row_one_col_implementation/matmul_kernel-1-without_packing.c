...
void kernel_6x16(float* blockA, float* blockB, float* C, const int mc, const int nc, const int kc) 
{
    ...
    for (int i = 0; i < mc; i++)
    {
        for (int j = 0; j < nc; j++)
        {
            ...
            for (int p = 0; p < kc; )
            {
                a0 = _mm256_loadu_ps(blockA(i, p, KDIM));
                b0 = _mm256_loadu_ps(blockB(p, j, KDIM));
...
//NOTE: In this trivial blocking strategy, no need to pack. Since the memory is already contiguous inside the block.
