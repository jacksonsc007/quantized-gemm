    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    for (int j = 0; j < block_n; j += 2)
    {
        ...
        for (int p = 0; p < block_k; p += Q_BLK_SIZE)
        {
            ...
            float sB0 = *blockSB(p / Q_BLK_SIZE, j, block_k/Q_BLK_SIZE);
            float sB1 = *blockSB(p / Q_BLK_SIZE, j+1, block_k/Q_BLK_SIZE);
            ...
            __m256i b_pack_int8_0 = _mm256_loadu_si256((__m256i *) blockB(p, j, block_k));
            __m256i b_pack_int8_1 = _mm256_loadu_si256((__m256i *) blockB(p, j+1, block_k));
