    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
        ...
        for (int p = 0; p < block_k; p += Q_BLK_SIZE)
        {
            __m256 fused_s = _mm256_set1_ps( blockSA * blockSB);
            ...
            sum_packed_fp_0_0 += tmp00 * fused_s;
            sum_packed_fp_0_1 += tmp01 * fused_s;
            sum_packed_fp_1_0 += tmp10 * fused_s;
            sum_packed_fp_1_1 += tmp11 * fused_s;
            sum_packed_fp_2_0 += tmp20 * fused_s;
            sum_packed_fp_2_1 += tmp21 * fused_s;
        }
...
int main() {
    ...
    // init_rand(A, M, K);
    // init_rand(B, K, N);
    init_int_const(A, 12, M, K);
    init_int_const(B, 12, K, N);
