...
void kernel_2x16(
    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    // Reduced register usage for 2x16 kernel
    __m256i a0, a1, b0, b1;
    __m256i c0_low, c0_high, c1_low, c1_high;
    
    // Precompute scaling factors outside inner loop
    __m256 sa[2], sb0, sb1;
    __m256 fused_s0[2], fused_s1[2];
    
    // Mask handling remains similar
    __m256i masks[2];
    __m256 c[2][2];
    
    // Mask initialization (same as original)
    if(block_n != NR) {
    ...
    for(int p=0; p<block_k; p+=Q_BLK_SIZE) {
        // Preload all scaling factors first
        sb0 = _mm256_load_ps(blockSB(p/Q_BLK_SIZE, 0, NR));
        sb1 = _mm256_load_ps(blockSB(p/Q_BLK_SIZE, 8, NR));
        sa[0] = _mm256_broadcast_ss(blockSA(0, p/Q_BLK_SIZE, MR));
        sa[1] = _mm256_broadcast_ss(blockSA(1, p/Q_BLK_SIZE, MR));
        
        // Precompute fused scales for both rows
        fused_s0[0] = _mm256_mul_ps(sa[0], sb0);
        fused_s1[0] = _mm256_mul_ps(sa[0], sb1);
        fused_s0[1] = _mm256_mul_ps(sa[1], sb0);
        fused_s1[1] = _mm256_mul_ps(sa[1], sb1);

        // Process both rows in single loop
        for(int q=0; q<Q_BLK_SIZE; q+=2) { // Unroll by 2
            // Load 2 consecutive B values (32 bytes)
            __m256i b_pack = _mm256_loadu_si256((__m256i*)blockB(p+q, 0, NR));
            
            // Process first row
            a0 = _mm256_set1_epi16(*(int16_t*)blockA(0, p+q, MR));
            __m256i prod0 = _mm256_maddubs_epi16(a0, b_pack);
            
            // Process second row
            a1 = _mm256_set1_epi16(*(int16_t*)blockA(1, p+q, MR));
            __m256i prod1 = _mm256_maddubs_epi16(a1, b_pack);

            // Efficient 16->32 expansion using permute
            c0_low = _mm256_cvtepi16_epi32(_mm256_castsi256_si128(prod0));
            c0_high = _mm256_cvtepi16_epi32(_mm256_extracti128_si256(prod0,1));
            c1_low = _mm256_cvtepi16_epi32(_mm256_castsi256_si128(prod1));
            c1_high = _mm256_cvtepi16_epi32(_mm256_extracti128_si256(prod1,1));

            // Accumulate results
            c[0][0] = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c0_low), fused_s0[0], c[0][0]);
            c[0][1] = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c0_high), fused_s1[0], c[0][1]);
            c[1][0] = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c1_low), fused_s0[1], c[1][0]);
            c[1][1] = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c1_high), fused_s1[1], c[1][1]);
        }
    }
