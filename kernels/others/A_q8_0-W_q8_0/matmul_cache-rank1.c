    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    ...
    for (int p = 0; p < block_k; p += Q_BLK_SIZE) 
    {
        ...
        for (int pq = 0; pq < Q_BLK_SIZE; pq++)
        {
            ...
            c0_0 += _mm256_cvtepi32_ps(c0_int_low) * fused_s00;
            c0_1 += _mm256_cvtepi32_ps(c0_int_high) * fused_s01;
            ...
            c1_0 += _mm256_cvtepi32_ps(c1_int_low) * fused_s10;
            c1_1 += _mm256_cvtepi32_ps(c1_int_high) * fused_s11;
        }
    }
