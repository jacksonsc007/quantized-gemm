...
void kernel_6x16(
    ...
    for (int p = 0; p < block_k; p += Q_BLK_SIZE) 
    {
        for (int q = 0; q < Q_BLK_SIZE; q += 1) 
        {
            ...
            a0 = _mm256_set1_epi16((int16_t) *blockA(1, p + q, MR));
            c0_int = _mm256_mullo_epi16(a0, b0);
            c0_int_low = _mm256_cvtepi16_epi32(_mm256_extracti128_si256(c0_int, 0));
            c0_int_high = _mm256_cvtepi16_epi32(_mm256_extracti128_si256(c0_int, 1));
            fused_s0 = _mm256_mul_ps(sa0, sb0);
...
void pack_blockA(int8_t* A, int8_t* blockA, int valid_mr)
{
...
void pack_sA(float* A, float* blockA, int valid_mr)
{
...
void matmul_kernel(
    ...
    for (int j = 0; j < N; j += NR) {
        ...
        for (int i = 0; i < M; i += MR) {
            ...
            pack_blockA(A(i, 0, K), blockA_packed, valid_mr);
            pack_sA(SA(i, 0, KDIM / Q_BLK_SIZE), sA_packed, valid_mr);
            kernel_6x16(blockA_packed, blockB_packed, sA_packed, sB_packed, C(i, j, N), valid_mr, valid_nr, K);
