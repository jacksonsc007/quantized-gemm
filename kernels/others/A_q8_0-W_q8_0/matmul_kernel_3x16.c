...
void kernel_2x16(
    ...
    __m256 c10 = _mm256_loadu_ps( C(1, 0, NDIM));
    __m256 c11 = _mm256_loadu_ps( C(1, 8, NDIM));
    __m256 c20 = _mm256_loadu_ps( C(2, 0, NDIM));
    __m256 c21 = _mm256_loadu_ps( C(2, 8, NDIM));
    ...
    for (int p = 0; p < block_k; p+= Q_BLK_SIZE)
    {
        for (int q = 0; q < Q_BLK_SIZE; q+= 1)
        {
            ...
            a0 = _mm256_set1_epi32(  (int32_t) *blockA(0, p+q, block_m));
            fused_s0 = _mm256_mul_ps(sa0, sb0);
            fused_s1 = _mm256_mul_ps(sa0, sb1);
            b0 = _mm256_cvtepi8_epi32(_mm_loadu_si64( (__m128i *) blockB(p+q, 0, block_n)));
            b1 = _mm256_cvtepi8_epi32(_mm_loadu_si64( (__m128i *) blockB(p+q, 8, block_n)));
            c0_int = _mm256_mullo_epi32(a0, b0);
            c1_int = _mm256_mullo_epi32(a0, b1);
            c00 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c0_int), fused_s0, c00);
            c01 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c1_int), fused_s1, c01);

            sa0 = _mm256_broadcast_ss( blockSA(1, p / Q_BLK_SIZE, block_m));
            a0 = _mm256_set1_epi32(  (int32_t) *blockA(1, p+q, block_m));
            fused_s0 = _mm256_mul_ps(sa0, sb0);
            fused_s1 = _mm256_mul_ps(sa0, sb1);
            b0 = _mm256_cvtepi8_epi32(_mm_loadu_si64( (__m128i *) blockB(p+q, 0, block_n)));
            b1 = _mm256_cvtepi8_epi32(_mm_loadu_si64( (__m128i *) blockB(p+q, 8, block_n)));
            c0_int = _mm256_mullo_epi32(a0, b0);
            c1_int = _mm256_mullo_epi32(a0, b1);
            c10 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c0_int), fused_s0, c10);
            c11 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c1_int), fused_s1, c11);

            sa0 = _mm256_broadcast_ss( blockSA(2, p / Q_BLK_SIZE, block_m));
            a0 = _mm256_set1_epi32(  (int32_t) *blockA(2, p+q, block_m));
            fused_s0 = _mm256_mul_ps(sa0, sb0);
            fused_s1 = _mm256_mul_ps(sa0, sb1);
            c0_int = _mm256_mullo_epi32(a0, b0);
            c1_int = _mm256_mullo_epi32(a0, b1);
            c20 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c0_int), fused_s0, c20);
            c21 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c1_int), fused_s1, c21);
        }
...
void matmul_kernel(
    ...
    for (int j = 0; j < N; j += NR) {
        ...
        for (int i = 0; i < M; i += MR) {
            ...
            kernel_2x16(blockA_packed, blockB_packed, sA_packed, sB_packed, C(i, j, N), MR, NR, K);
