...
void kernel_5x16(
    ...
    __m256 c30 = _mm256_loadu_ps(C(3, 0, NDIM));
    __m256 c31 = _mm256_loadu_ps(C(3, 8, NDIM));
    __m256 c40 = _mm256_loadu_ps(C(4, 0, NDIM));
    __m256 c41 = _mm256_loadu_ps(C(4, 8, NDIM));
    // __m256 c50 = _mm256_loadu_ps(C(5, 0, NDIM));
    // __m256 c51 = _mm256_loadu_ps(C(5, 8, NDIM));
    ...
    for (int p = 0; p < block_k; p += Q_BLK_SIZE) {
        for (int q = 0; q < Q_BLK_SIZE; q += 1) {
            ...
            fused_s0 = _mm256_mul_ps(sa0, sb0);
            fused_s1 = _mm256_mul_ps(sa0, sb1);
            c10 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c0_int_low), fused_s0, c10);
            c11 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c0_int_high), fused_s1, c11);
            ...
            fused_s0 = _mm256_mul_ps(sa0, sb0);
            fused_s1 = _mm256_mul_ps(sa0, sb1);
            c20 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c0_int_low), fused_s0, c20);
            c21 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(c0_int_high), fused_s1, c21);
...
void matmul_kernel(
    ...
    for (int j = 0; j < N; j += NR) {
        ...
        for (int i = 0; i < M; i += MR) {
            ...
            kernel_5x16(blockA_packed, blockB_packed, sA_packed, sB_packed, C(i, j, N), MR, NR, K);
