    ...
    for (int j = 0; j < block_n; ++j)
    {
        ...
        for (int p = 0; p < block_k; p += Q_BLK_SIZE)
        {
            ...
            // so ymm only need to load once
                ...
                __m256i sum_packed_0 = _mm256_dpbusd_epi32(_mm256_setzero_si256(),a_pack_int8_0, b_pack_int8);
                __m256i sum_packed_1 = _mm256_dpbusd_epi32(_mm256_setzero_si256(),a_pack_int8_1, b_pack_int8);
                // convert to fp32
                ...
                // sum_packed_fp_0 += tmp0 * _mm256_set1_ps(fused_s0);
                // sum_packed_fp_1 += tmp1 * _mm256_set1_ps(fused_s1);
        }
        __m128 tmp0 = _mm_add_ps(_mm256_extractf128_ps(sum_packed_fp_0, 1), _mm256_extractf128_ps(sum_packed_fp_0, 0));
        tmp0 = _mm_add_ps(tmp0, _mm_movehl_ps(tmp0, tmp0));
        tmp0 = _mm_add_ss(tmp0, _mm_movehdup_ps(tmp0));
        *C(0, j, NDIM) += _mm_cvtss_f32(tmp0);
        __m128 tmp1 = _mm_add_ps(_mm256_extractf128_ps(sum_packed_fp_1, 1), _mm256_extractf128_ps(sum_packed_fp_1, 0));
