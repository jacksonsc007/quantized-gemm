...
#define QK4_0_ink 64
typedef struct {
    ggml_half d1; // delta for first 32 elements
    ggml_half d2; // delta for second 32 elements
    uint8_t qs[QK4_0_ink / 2]; // nibbles / quants
} block_q4_0_ink;
static_assert(sizeof(block_q4_0_ink) == 2 * sizeof(ggml_half) + QK4_0_ink / 2, "wrong q4_0 block size/padding for ink implementation");
...
typedef struct {
    ...
    ggml_half d2; // delta for second 32 elements
    ggml_half m2; // min
    uint8_t qs[QK4_1_ink / 2]; // nibbles / quants
} block_q4_1_ink;
static_assert(sizeof(block_q4_1_ink) == 4 * sizeof(ggml_half) + QK4_1_ink / 2, "wrong q4_1 block size/padding for ink implementation");

typedef struct {
    float d1; // delta for first 32 elements
    float m1; // min
    float d2; // delta for second 32 elements
    float m2; // min
    uint8_t qs[QK4_1_ink / 2]; // nibbles / quants
} block_q4_1_ink_fp32;
static_assert(sizeof(block_q4_1_ink_fp32) == 4 * sizeof(float) + QK4_1_ink / 2, "wrong q4_1 block size/padding for fp32 ink implementation");

#define QK8_0_ink 64
typedef struct {
    ggml_half d1; // delta
    ggml_half d2; // delta
    int8_t qs1[QK8_0_ink / 2]; // quants of first 32 elements
    int8_t qs2[QK8_0_ink / 2]; // quants of second 32 elements
} block_q8_0_ink;
static_assert(sizeof(block_q8_0_ink) == 2*sizeof(ggml_half) + QK8_0_ink, "wrong q8_0 block size/padding for ink implementation");


#define QK8_1_ink 64
typedef struct {
    ggml_half d1; // delta
    ggml_half s1; // d * sum(qs[i])
    ggml_half d2; // delta
    ggml_half s2; // d * sum(qs[i])
    int8_t qs1[QK8_1_ink / 2]; // quants of first 32 elements
    int8_t qs2[QK8_1_ink / 2]; // quants of second 32 elements
} block_q8_1_ink;
static_assert(sizeof(block_q8_1_ink) == 4*sizeof(ggml_half) + QK8_1_ink, "wrong q8_1 block size/padding for ink implementation");
...
static_assert(sizeof(block_q8_1_ink_fp32) == 4*sizeof(float) + QK8_1_ink, "wrong q8_1 block size/padding for fp32 ink implementation");
