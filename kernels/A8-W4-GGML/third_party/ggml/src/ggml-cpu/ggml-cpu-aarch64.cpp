...
static void ggml_gemm_q4_0_8x8_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    ...
    // const std::string profile_name = "ggml_gemm_q4_0_8x8_q8_0: ";
...
template <typename BLOC_TYPE, int64_t INTER_SIZE, int64_t NB_COLS, ggml_type PARAM_TYPE> class tensor_traits : public tensor_traits_base {
    ...
    void forward_mul_mat(ggml_compute_params * params, ggml_tensor * op) {
        ...
        // Ink: Add Performance Profling
        const long long  m = ne11;
        const long long  k = ne10;
        const long long  n = ne01;
        const long long bs = ne12;
        const long long flops = (m * n * 2 * k) * bs; 
        const std::string profile_name = "forward_mul_mat";
        std::ostringstream oss;
        oss << "[" << profile_name << " (ith=" << ith << ") : " << m << " x " << n << " x " << k << "]";
        std::string formatted_profile_name = oss.str();
