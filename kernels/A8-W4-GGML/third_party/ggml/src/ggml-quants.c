...
void quantize_row_q4_1_ink_fp32_ref(const float * GGML_RESTRICT x, block_q4_1_ink_fp32 * GGML_RESTRICT y, int64_t k) {
    const int blocksize = QK4_1_ink;
    const int sub_blocksize = QK4_1_ink / 2;

    assert(k % blocksize == 0);
    assert(blocksize == 64);
    assert(sub_blocksize == 32);

    const int nb = k / blocksize;

    for (int i = 0; i < nb; i++) {
        float tmp_id[2];
        float tmp_min[2];
        // deal with first sub_block with 32 elements
        {
            int index = 2 * i;
            float min = FLT_MAX;
            float max = -FLT_MAX;

            for (int j = 0; j < sub_blocksize; j++) {
                const float v = x[index * sub_blocksize + j];
        ...
        // deal with second sub_block 32 elements
        {
            int index = 2 * i + 1;
            float min = FLT_MAX;
            float max = -FLT_MAX;
        ...
        for (int j = 0; j < sub_blocksize; ++j) {
            // get elements from 1st sub_block
            const float x0 = (x[i * blocksize + 0    + j] - tmp_min[0]) * tmp_id[0];
            // get elements from 2nd sub_block
            const float x1 = (x[i * blocksize + blocksize/2 + j] - tmp_min[1]) * tmp_id[1];
...
void quantize_row_q4_0_ink_ref(const float * GGML_RESTRICT x, block_q4_0_ink * GGML_RESTRICT y, int64_t k) {
    const int blocksize = QK4_1_ink;
    const int sub_blocksize = QK4_1_ink / 2;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < sub_blocksize; ++j) {
            // get elements from 1st sub_block
            const float x0 = (x[i * blocksize + 0    + j] ) * tmp_id[0];
            // get elements from 2nd sub_block
            const float x1 = (x[i * blocksize + blocksize/2 + j] ) * tmp_id[1];

            // TODO: try to discard 8
            const uint8_t xi0 = MIN(15, (int8_t)(x0 + 8.5f));
            const uint8_t xi1 = MIN(15, (int8_t)(x1 + 8.5f));
...
void quantize_row_q4_1_ink_ref(const float * GGML_RESTRICT x, block_q4_1_ink * GGML_RESTRICT y, int64_t k) {
    const int blocksize = QK4_1_ink;
    const int sub_blocksize = QK4_1_ink / 2;
...
void quantize_row_q8_0_ink_ref(const float * GGML_RESTRICT x, block_q8_0_ink * GGML_RESTRICT y, int64_t k) {

    assert(QK8_0_ink == 64);
    assert(k % QK8_0_ink == 0);
    const int nb = k / QK8_0_ink;
    const int half_QK8_0_ink = QK8_0_ink/2;
    for (int i = 0; i < nb; i++) 
    {
        // first 32 elements
        {         
            ...
            for (int j = 0; j < half_QK8_0_ink; j++) {
                const float v = x[index*half_QK8_0_ink + j];
                amax = MAX(amax, fabsf(v));
            }
            ...
            for (int j = 0; j < half_QK8_0_ink; ++j) {
                const float x0 =  x[index * half_QK8_0_ink + j] * id;
                y[i].qs1[j] = roundf( x0 );
            }
        }
        // second 32 elements
...
void quantize_row_q8_1_ink_fp32(const float * GGML_RESTRICT x, block_q8_1_ink_fp32 * GGML_RESTRICT y, int64_t k) {
    assert(QK8_1_ink == 64);
    assert(k % QK8_1_ink == 0);
    const int nb = k / QK8_1_ink;
    const int half_QK8_1_ink = QK8_1_ink/2;
    for (int i = 0; i < nb; i++) 
    {
        // first 32 elements
        {         
            ...
            for (int j = 0; j < half_QK8_1_ink; j++) {
                const float v = x[index*half_QK8_1_ink + j];
                amax = MAX(amax, fabsf(v));
            }
            ...
            for (int j = 0; j < half_QK8_1_ink; ++j) {
                const float v0 = x[index*half_QK8_1_ink           + j]*id;
...
void quantize_row_q8_1_ink(const float * GGML_RESTRICT x, block_q8_1_ink * GGML_RESTRICT y, int64_t k) {
    assert(QK8_1_ink == 64);
    assert(k % QK8_1_ink == 0);
    const int nb = k / QK8_1_ink;
    const int half_QK8_1_ink = QK8_1_ink/2;
    for (int i = 0; i < nb; i++) 
    {
        ...
        {         
            ...
            y[i].s1 = GGML_FP32_TO_FP16(sum*d); 
        }
        // second 32 elements
        {         
            float amax = 0.0f; // absolute max
            ...
            y[i].s2 = GGML_FP32_TO_FP16(sum*d); 
        }
    }
}
...
void dequantize_row_q4_0_ink(const block_q4_0_ink * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    const int blocksize = QK4_0_ink;
    const int sub_blocksize = QK4_0_ink / 2;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        const float d2 = GGML_FP16_TO_FP32(x[i].d2);

        for (int j = 0; j < sub_blocksize; ++j) {
            const int x0 = (x[i].qs[j] & 0x0F) - 8;
            const int x1 = (x[i].qs[j] >>   4) - 8;

            y[i*blocksize + j + 0   ] = x0*d1;
            y[i*blocksize + j + sub_blocksize] = x1*d2;
        }
    }
}

void dequantize_row_q4_1_ink(const block_q4_1_ink * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    const int blocksize = QK4_1_ink;
    const int sub_blocksize = QK4_1_ink / 2;
    ...
    for (int i = 0; i < nb; i++) {
        const float d1 = GGML_FP16_TO_FP32(x[i].d1);
        const float m1 = GGML_FP16_TO_FP32(x[i].m1);
        const float d2 = GGML_FP16_TO_FP32(x[i].d2);
        const float m2 = GGML_FP16_TO_FP32(x[i].m2);

        for (int j = 0; j < sub_blocksize; ++j) {
            const int x0 = (x[i].qs[j] & 0x0F);
            const int x1 = (x[i].qs[j] >>   4);

            y[i*blocksize + j + 0   ] = x0*d1 + m1;
            y[i*blocksize + j + sub_blocksize] = x1*d2 + m2;
        }
    }
}

void dequantize_row_q4_1_ink_fp32(const block_q4_1_ink_fp32 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    const int blocksize = QK4_1_ink;
    const int sub_blocksize = QK4_1_ink / 2;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        const float m1 = (x[i].m1);
        const float d2 = (x[i].d2);
        const float m2 = (x[i].m2);
...
void dequantize_row_q8_0_ink(const block_q8_0_ink * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    const int blocksize = QK8_0_ink;
    const int half_blocksize = QK8_0_ink / 2;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < half_blocksize; ++j) {
            y[i*blocksize + j] = x[i].qs1[j]*d1;
            y[i*blocksize + j + half_blocksize] = x[i].qs2[j]*d2;
        }
    }
}
...
// NOTE: ink modified
size_t quantize_q4_0_ink(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
        quantize_row_q4_0_ink_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * ggml_row_size(GGML_TYPE_Q4_0_ink, n_per_row);
    }
    else
    {
        // raise not implemented error
        fprintf(stderr, "Error: quantize_q4_1_ink is not implemented yet.\n");
        return 0;
    }
}

size_t quantize_q4_1_ink(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
        quantize_row_q4_1_ink_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * ggml_row_size(GGML_TYPE_Q4_1_ink, n_per_row);
    }
    else
    {
        // raise not implemented error
...
size_t quantize_q4_1_ink_fp32(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
        quantize_row_q4_1_ink_fp32_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * ggml_row_size(GGML_TYPE_Q4_1_ink_fp32, n_per_row);
    }
    else
    {
        // raise not implemented error
...
size_t quantize_q8_0_ink(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights; // not used
    const size_t row_size = ggml_row_size(GGML_TYPE_Q8_0_ink, n_per_row);
    quantize_row_q8_0_ink_ref(src, dst, (int64_t)nrow*n_per_row);
    return nrow * row_size;
}

size_t quantize_q8_1(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights; // not used
...
size_t quantize_q8_1_ink(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights; // not used
    const size_t row_size = ggml_row_size(GGML_TYPE_Q8_1_ink, n_per_row);
    quantize_row_q8_1_ink(src, dst, (int64_t)nrow*n_per_row);
    return nrow * row_size;
}

size_t quantize_q8_1_ink_fp32(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights; // not used
    const size_t row_size = ggml_row_size(GGML_TYPE_Q8_1_ink_fp32, n_per_row);
    quantize_row_q8_1_ink_fp32(src, dst, (int64_t)nrow*n_per_row);
    return nrow * row_size;
}
