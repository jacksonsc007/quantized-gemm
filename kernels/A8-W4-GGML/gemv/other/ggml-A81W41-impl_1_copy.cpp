...
int main() {
    ...
    for (int block_idx = 0; block_idx < num_q8_blocks; ++block_idx) {
        ...
        float scale = GGML_FP16_TO_FP32(blk.d);
        float scaled_sum = GGML_FP16_TO_FP32(blk.s);
        sA[block_idx] = scale;
        scaledSumA[block_idx] = scaled_sum;
        memcpy(A + block_idx * QK8_1, blk.qs, QK8_1 * sizeof(int8_t));
    }
    ...
    for (int block_idx = 0; block_idx < num_q4_blocks; ++block_idx) {
        ...
        float scale = GGML_FP16_TO_FP32(blk.d);
        float min_value = GGML_FP16_TO_FP32(blk.m);
        sB[block_idx] = scale;
        MinB[block_idx] = min_value;
        memcpy(B + block_idx * QK4_1 / 2, blk.qs, QK4_1 / 2 * sizeof(uint8_t));
    }
    ...
    printf("\n\e[32m[INFO]\e[m llama.cpp q41q81 implementation 1\n");
