...
int main() {
    ...
    // ggml initialization
    const ggml_type weight_q_type = GGML_TYPE_Q4_1_ink_fp32;
    const ggml_type activation_q_type = GGML_TYPE_Q8_1_ink_fp32;
    ...
    block_q8_1_ink_fp32 * q8_blocks = (block_q8_1_ink_fp32 *)q8_activation->data;
    ...
    for (int block_idx = 0; block_idx < num_q8_blocks; ++block_idx) {
        block_q8_1_ink_fp32 blk = q8_blocks[block_idx];
        float scale_1      = (blk.d1);
        float scaled_sum_1 = (blk.s1);
        float scale_2      = (blk.d2);
        float scaled_sum_2 = (blk.s2);
    ...
    block_q4_1_ink_fp32 * q4_blocks = (block_q4_1_ink_fp32 *)q4_weight->data;
    int32_t num_q4_blocks = q4_weight->ne[0] * q4_weight->ne[1] / QK4_1_ink;
    for (int block_idx = 0; block_idx < num_q4_blocks; ++block_idx) {
        block_q4_1_ink_fp32 blk = q4_blocks[block_idx];
        float scale_1     = (blk.d1);
        float min_value_1 = (blk.m1);
        float scale_2     = (blk.d2);
        float min_value_2 = (blk.m2);
    ...
    printf("\n\e[32m[INFO]\e[m llama.cpp q41q81 ink modified, fp32\n");
