    ...
    const int M, const int N, const int K) 
{
    ...
    for (int j = 0; j < N; j++)
    {
        ...
        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            ...
            float sA_0 = GGML_FP16_TO_FP32( *SA(0            , block_idx    , KDIM / Q_BLK_SIZE) ); 
            float sA_1 = GGML_FP16_TO_FP32( *SA(0            , block_idx + 1, KDIM / Q_BLK_SIZE) ); 
            float sB_0 = GGML_FP16_TO_FP32( *SB(block_idx    , j            , KDIM / Q_BLK_SIZE) ); 
            float sB_1 = GGML_FP16_TO_FP32( *SB(block_idx + 1, j            , KDIM / Q_BLK_SIZE) ); 
            ...
            float sum_a_0 = GGML_FP16_TO_FP32( *ScaledSumA ( 0            , block_idx    , KDIM / Q_BLK_SIZE ) ); 
            float sum_a_1 = GGML_FP16_TO_FP32( *ScaledSumA ( 0            , block_idx + 1, KDIM / Q_BLK_SIZE ) ); 
            ...
            extra_sum_fp += sum_a_0 * min_b_0;
            extra_sum_fp += sum_a_1 * min_b_1;
        }
