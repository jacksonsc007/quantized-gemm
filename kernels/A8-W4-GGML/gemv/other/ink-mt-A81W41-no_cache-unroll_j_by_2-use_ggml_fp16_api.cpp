...
void matmul_kernel(
    ...
    float16* SA, float16* ScaledSumA, float16* SB, float16* MinB,
    const int M, const int N, const int K) 
{
    ...
    for (int j = 0; j < N - 1; j += 2)
    {
        ...
        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            ...
            float sA_0    = GGML_FP16_TO_FP32( *SA ( 0            , block_idx    , K/Q_BLK_SIZE ) ); 
            float sB_0_j0 = GGML_FP16_TO_FP32( *SB ( block_idx    , j            , K/Q_BLK_SIZE ) ); 
            float sB_0_j1 = GGML_FP16_TO_FP32( *SB ( block_idx    , j+1          , K/Q_BLK_SIZE ) ); 
            float sA_1    = GGML_FP16_TO_FP32( *SA ( 0            , block_idx + 1, K/Q_BLK_SIZE ) ); 
            float sB_1_j0 = GGML_FP16_TO_FP32( *SB ( block_idx + 1, j            , K/Q_BLK_SIZE ) ); 
            float sB_1_j1 = GGML_FP16_TO_FP32( *SB ( block_idx + 1, j+1          , K/Q_BLK_SIZE ) );
            ...
            float min_b_0_j0    = GGML_FP16_TO_FP32( *MinB       ( block_idx    , j            , K/Q_BLK_SIZE ) ); 
            float min_b_0_j1    = GGML_FP16_TO_FP32( *MinB       ( block_idx    , j+1          , K/Q_BLK_SIZE ) ); 
            float min_b_1_j0    = GGML_FP16_TO_FP32( *MinB       ( block_idx + 1, j            , K/Q_BLK_SIZE ) ); 
            float min_b_1_j1    = GGML_FP16_TO_FP32( *MinB       ( block_idx + 1, j+1          , K/Q_BLK_SIZE ) ); 
            float sum_a_0_fused = GGML_FP16_TO_FP32( *ScaledSumA ( 0            , block_idx    , K/Q_BLK_SIZE ) ); 
            float sum_a_1_fused = GGML_FP16_TO_FP32( *ScaledSumA ( 0            , block_idx + 1, K/Q_BLK_SIZE ) ); 
        ...
        *C(0, j, NDIM) += hsum_float_8(sum_packed_fp_j0) + extra_sum_fp_j0;
        *C(0, j+1, NDIM) += hsum_float_8(sum_packed_fp_j1) + extra_sum_fp_j1;
    }
...
void matmul_naive_no_packing(
    ...
    for (int j = 0; j < N; ++j)
    {
        ...
        for (int p = 0; p < K; p += Q_BLK_SIZE * 2)
        {
            ...
            int block_idx = p / Q_BLK_SIZE;
            float s_a_0 = GGML_FP16_TO_FP32( *SA(i            , block_idx    , KDIM / Q_BLK_SIZE) ); 
            float s_a_1 = GGML_FP16_TO_FP32( *SA(i            , block_idx + 1, KDIM / Q_BLK_SIZE) ); 
            float s_b_0 = GGML_FP16_TO_FP32( *SB(block_idx    , j            , KDIM / Q_BLK_SIZE) ); 
            float s_b_1 = GGML_FP16_TO_FP32( *SB(block_idx + 1, j            , KDIM / Q_BLK_SIZE) ); 
                
                
            float min_b_0 = GGML_FP16_TO_FP32( *MinB       ( block_idx    , j            , KDIM / Q_BLK_SIZE ) ); 
            float min_b_1 = GGML_FP16_TO_FP32( *MinB       ( block_idx + 1, j            , KDIM / Q_BLK_SIZE ) ); 
            float sum_a_0 = GGML_FP16_TO_FP32( *ScaledSumA ( i            , block_idx    , KDIM / Q_BLK_SIZE ) ); 
            float sum_a_1 = GGML_FP16_TO_FP32( *ScaledSumA ( i            , block_idx + 1, KDIM / Q_BLK_SIZE ) ); 
...
int main() {
    ...
    float16* sA  = (float16*)_mm_malloc(M * K / Q_BLK_SIZE * sizeof(float16), MEM_ALIGN);
    float16* scaledSumA  = (float16*)_mm_malloc(M * K / Q_BLK_SIZE * sizeof(float16), MEM_ALIGN);
    float16* sB  = (float16*)_mm_malloc(K * N / Q_BLK_SIZE * sizeof(float16), MEM_ALIGN);
    float16* MinB  = (float16*)_mm_malloc(K * N / Q_BLK_SIZE * sizeof(float16), MEM_ALIGN);
    ...
    for (int block_idx = 0; block_idx < num_q8_blocks; ++block_idx) {
        ...
        float16 scale_1      = (blk.d1);
        float16 scaled_sum_1 = (blk.s1);
        float16 scale_2      = (blk.d2);
        float16 scaled_sum_2 = (blk.s2);
    ...
    for (int block_idx = 0; block_idx < num_q4_blocks; ++block_idx) {
        ...
        float16 scale_1     = (blk.d1);
        float16 min_value_1 = (blk.m1);
        float16 scale_2     = (blk.d2);
        float16 min_value_2 = (blk.m2);
...
#ifdef TEST
        int res = compare_mats(C, C_ref, (long)M * (long)N);
        // if (!res) break;
#endif
        printf("\n");
    }
