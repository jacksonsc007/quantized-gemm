    ...
    const int M, const int N, const int K) 
{
    ...
    for (int j = 0; j < N; j += 2)
    {
        ...
        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            ...
            sum_packed_fp_00 = _mm256_fmadd_ps(tmp_0_j0, _mm256_set1_ps(fused_s_0_j0), sum_packed_fp_00);
            sum_packed_fp_01 = _mm256_fmadd_ps(tmp_1_j0, _mm256_set1_ps(fused_s_1_j0), sum_packed_fp_01);
            ...
            sum_packed_fp_10 = _mm256_fmadd_ps(tmp_0_j1, _mm256_set1_ps(fused_s_0_j1), sum_packed_fp_10);
            sum_packed_fp_11 = _mm256_fmadd_ps(tmp_1_j1, _mm256_set1_ps(fused_s_1_j1), sum_packed_fp_11);
        }
        
        // Store results
        *C(0, j, NDIM) += hsum_float_8(sum_packed_fp_00);
        *C(0, j, NDIM) += hsum_float_8(sum_packed_fp_01);
        *C(0, j+1, NDIM) += hsum_float_8(sum_packed_fp_10);
        *C(0, j+1, NDIM) += hsum_float_8(sum_packed_fp_11);
    }
}
void matmul_naive_no_packing(
    ...
    for (int j = 0; j < N; ++j)
    {
        ...
        for (int p = 0; p < K; p += Q_BLK_SIZE * 2)
        {
            // SA: row-major
            // SB: column-major
            long long sum_int8_0 = 0, sum_int8_1 = 0;
            int block_idx = p / Q_BLK_SIZE;
...
int main() {
    ...
    printf("\n\e[32m[INFO]\e[m ink q40q80, use ggml fp16\n");
