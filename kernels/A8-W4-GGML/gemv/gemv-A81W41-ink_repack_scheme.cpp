...
)
{
    ...
    struct q8_repack_1x2_fp32 * A_ptr = A_ptr_start;
...
void quantize_row_q8_1_repack(const float * x, void * vy, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        y[i].scaled_sum_low = d1 * hsum_i32_8(_mm256_add_epi32(_mm256_add_epi32(i0, i1), _mm256_add_epi32(i2, i3)));
        y[i].scaled_sum_high = d2 * hsum_i32_8(_mm256_add_epi32(_mm256_add_epi32(i4, i5), _mm256_add_epi32(i6, i7)));
...
int main() {
    ...
    */
    ...
    struct q8_repack_1x2_fp32 * A_repack_start = (struct q8_repack_1x2_fp32 *) A_repack;
    ...
    if (repack_during_quantization)
    {
        ...
        for (int i = 0; i < M; i++) 
        {
            struct q8_repack_1x2_fp32 * A_ptr = A_repack_start + i * num_repack_blk_A_along_K;
