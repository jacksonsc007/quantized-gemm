    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    ...
    for (int j = 0; j < block_n; ++j)
    {
        ...
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            ...
            // Load scale factors for all 4 i iterations
            float sA_0[4], sA_1[4], sum_a_0_fused[4], sum_a_1_fused[4];
            for (int ii = 0; ii < 4; ii++) {
                sA_0[ii] = *blockSA(i + ii, block_idx, block_k/Q_BLK_SIZE);
                sA_1[ii] = *blockSA(i + ii, block_idx + 1, block_k/Q_BLK_SIZE);
                sum_a_0_fused[ii] = *blockScaledSumA(i + ii, block_idx, block_k / Q_BLK_SIZE);
                sum_a_1_fused[ii] = *blockScaledSumA(i + ii, block_idx + 1, block_k / Q_BLK_SIZE);
            }
            
            float sB_0 = *blockSB(block_idx, j, block_k/Q_BLK_SIZE);
            float sB_1 = *blockSB(block_idx + 1, j, block_k/Q_BLK_SIZE);
            float fused_s_0 = sB_0;  // Will be multiplied by sA_0[ii] later
            float fused_s_1 = sB_1;  // Will be multiplied by sA_1[ii] later

            float min_b_0 = *blockMinB(block_idx, j, block_k / Q_BLK_SIZE);
            float min_b_1 = *blockMinB(block_idx + 1, j, block_k / Q_BLK_SIZE);

            // Load A blocks for all 4 i iterations
            __m256i a_pack_int8_low[4], a_pack_int8_high[4];
            for (int ii = 0; ii < 4; ii++) {
                a_pack_int8_low[ii] = _mm256_loadu_si256((__m256i *) blockA(i + ii, p, block_k));
                a_pack_int8_high[ii] = _mm256_loadu_si256((__m256i *) blockA(i + ii, p + Q_BLK_SIZE, block_k));
            }

            // Load B block (shared across all i iterations)
            ...
            // Process all 4 i iterations
            for (int ii = 0; ii < 4; ii++) {
                __m256 tmp_0 = mul_sum_us8_pairs_float(b_pack_int8_low, a_pack_int8_low[ii]);
                __m256 tmp_1 = mul_sum_us8_pairs_float(b_pack_int8_high, a_pack_int8_high[ii]);
                
                // Multiply by the per-i scale factors
                __m256 fused_s_0_vec = _mm256_set1_ps(sA_0[ii] * fused_s_0);
                __m256 fused_s_1_vec = _mm256_set1_ps(sA_1[ii] * fused_s_1);
                
                // Accumulate results
                __m256* sum_packed_fp;
                switch(ii) {
                    case 0: sum_packed_fp = &sum_packed_fp_0; break;
                    case 1: sum_packed_fp = &sum_packed_fp_1; break;
                    case 2: sum_packed_fp = &sum_packed_fp_2; break;
                    case 3: sum_packed_fp = &sum_packed_fp_3; break;
                }
                
                *sum_packed_fp = _mm256_fmadd_ps(tmp_0, fused_s_0_vec, *sum_packed_fp);
                *sum_packed_fp = _mm256_fmadd_ps(tmp_1, fused_s_1_vec, *sum_packed_fp);
                
                // Accumulate extra sums
                float* extra_sum_fp;
                switch(ii) {
                    case 0: extra_sum_fp = &extra_sum_fp_0; break;
                    case 1: extra_sum_fp = &extra_sum_fp_1; break;
                    case 2: extra_sum_fp = &extra_sum_fp_2; break;
                    case 3: extra_sum_fp = &extra_sum_fp_3; break;
                }
                
                *extra_sum_fp += sum_a_0_fused[ii] * min_b_0;
                *extra_sum_fp += sum_a_1_fused[ii] * min_b_1;
            }
        }
        
        // Store results for all 4 i iterations
        for (int ii = 0; ii < 4; ii++) {
            if (i + ii >= block_m) break;  // Handle boundary condition
            ...
            switch(ii) {
                case 0: 
                    sum_packed_fp = sum_packed_fp_0;
                    extra_sum_fp = extra_sum_fp_0;
                    break;
                case 1: 
                    sum_packed_fp = sum_packed_fp_1;
                    extra_sum_fp = extra_sum_fp_1;
                    break;
                case 2: 
                    sum_packed_fp = sum_packed_fp_2;
                    extra_sum_fp = extra_sum_fp_2;
                    break;
                case 3: 
                    sum_packed_fp = sum_packed_fp_3;
                    extra_sum_fp = extra_sum_fp_3;
                    break;
            }
            
            *C(i + ii, j, NDIM) += (hsum_float_8(sum_packed_fp) + extra_sum_fp);
        }
    }
}
