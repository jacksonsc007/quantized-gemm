    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    ...
    for (int j = 0; j < block_n; ++j)
    {
        ...
        __m256 extra_sum_packed_fp = _mm256_setzero_ps();
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            ...
            // so ymm only need to load once
                ...
                extra_sum_packed_fp = _mm256_fmadd_ps(_mm256_set1_ps(min_b_0), sum_a_0_fused, extra_sum_packed_fp);
                extra_sum_packed_fp = _mm256_fmadd_ps(_mm256_set1_ps(min_b_1), sum_a_1_fused, extra_sum_packed_fp);
        ...
        *C(i, j, NDIM) +=  hsum_float_8(extra_sum_packed_fp);
