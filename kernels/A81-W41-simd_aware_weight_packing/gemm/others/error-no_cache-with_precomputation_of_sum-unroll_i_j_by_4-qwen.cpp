...
{
    // Assume M and N are multiples of 4; otherwise, add cleanup loops
    for (int jj = 0; jj < N; jj += 4)
    for (int ii = 0; ii < M; ii += 4)
    {
        // Initialize 4x4 accumulators
        __m256 sum_packed_fp[4][4];
        float extra_sum_fp[4][4] = {0};

        // Zero-initialize all 16 vectors
        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j)
                sum_packed_fp[i][j] = _mm256_setzero_ps();

        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            int block_idx = p / Q_BLK_SIZE;

            // Load scale factors for all 4 rows and 4 cols
            float sA[4], sB[4][4], fused_s[4][4];
            float min_b[4][4];
            float sum_a_fused[4][4];

            // Load SA for 4 rows
            for (int i = 0; i < 4; ++i)
                sA[i] = *SA(ii + i, block_idx, K/Q_BLK_SIZE);

            // Load SB, MinB, ScaledSumA for 4 columns
            for (int j = 0; j < 4; ++j) {
                for (int i = 0; i < 4; ++i) {
                    sB[i][j] = *SB(block_idx, jj + j, K/Q_BLK_SIZE);
                    min_b[i][j] = *MinB(block_idx, jj + j, K/Q_BLK_SIZE);
                    fused_s[i][j] = sA[i] * sB[i][j];
                    sum_a_fused[i][j] = *ScaledSumA(ii + i, block_idx, K/Q_BLK_SIZE);
                }
            }

            // Load A for two blocks per row
            __m256i a_pack_int8_low[4], a_pack_int8_high[4];
            for (int i = 0; i < 4; ++i) {
                a_pack_int8_low[i] = _mm256_loadu_si256((__m256i *) A(ii + i, p, K));
                a_pack_int8_high[i] = _mm256_loadu_si256((__m256i *) A(ii + i, p + Q_BLK_SIZE, K));
            }

            // Load B for current block
            __m256i b_pack_int8 = _mm256_loadu_si256((__m256i *) B(p/2, jj, K/2));
            ...
            // Compute dot products for low part
            for (int i = 0; i < 4; ++i) {
                __m256 tmp = mul_sum_us8_pairs_float(b_pack_int8_low, a_pack_int8_low[i]);
                for (int j = 0; j < 4; ++j) {
                    sum_packed_fp[i][j] = _mm256_fmadd_ps(tmp, _mm256_set1_ps(fused_s[i][j]), sum_packed_fp[i][j]);
                }
            }

            // Compute dot products for high part
            for (int i = 0; i < 4; ++i) {
                __m256 tmp = mul_sum_us8_pairs_float(b_pack_int8_high, a_pack_int8_high[i]);
                for (int j = 0; j < 4; ++j) {
                    sum_packed_fp[i][j] = _mm256_fmadd_ps(tmp, _mm256_set1_ps(fused_s[i][j]), sum_packed_fp[i][j]);
                }
            }

            // Accumulate extra sum terms
            for (int i = 0; i < 4; ++i)
                for (int j = 0; j < 4; ++j)
                    extra_sum_fp[i][j] += sum_a_fused[i][j] * min_b[i][j];
        }

        // Final accumulation and store
        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j) {
                float total = hsum_float_8(sum_packed_fp[i][j]) + extra_sum_fp[i][j];
                C[(ii + i)*NDIM + (jj + j)] += total;
            }
    }
}
