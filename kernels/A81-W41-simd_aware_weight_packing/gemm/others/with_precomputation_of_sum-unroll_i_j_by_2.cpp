...
void kernel(
    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    for (int i = 0; i < block_m; i += 2)
    for (int j = 0; j < block_n; j += 2)
    {
        // Accumulators for i, j
        ...
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            ...
            // Load scale factors for A (same for all j)
            float sA_0_0 = *blockSA(i, block_idx, block_k/Q_BLK_SIZE);
            float sA_0_1 = *blockSA(i, block_idx + 1, block_k/Q_BLK_SIZE);
            float sA_1_0 = *blockSA(i+1, block_idx, block_k/Q_BLK_SIZE);
            float sA_1_1 = *blockSA(i+1, block_idx + 1, block_k/Q_BLK_SIZE);
            
            // Load scale factors and min for B (j and j+1)
            float sB_0_j0 = *blockSB(block_idx, j, block_k/Q_BLK_SIZE);
            float sB_1_j0 = *blockSB(block_idx + 1, j, block_k/Q_BLK_SIZE);
            float sB_0_j1 = *blockSB(block_idx, j+1, block_k/Q_BLK_SIZE);
            float sB_1_j1 = *blockSB(block_idx + 1, j+1, block_k/Q_BLK_SIZE);

            float min_b_0_j0 = *blockMinB(block_idx, j, block_k / Q_BLK_SIZE);
            float min_b_1_j0 = *blockMinB(block_idx + 1, j, block_k / Q_BLK_SIZE);
            float min_b_0_j1 = *blockMinB(block_idx, j+1, block_k / Q_BLK_SIZE);
            float min_b_1_j1 = *blockMinB(block_idx + 1, j+1, block_k / Q_BLK_SIZE);

            // Fused scales for all combinations
            float fused_s_00_0 = sA_0_0 * sB_0_j0;
            float fused_s_00_1 = sA_0_1 * sB_1_j0;
            float fused_s_01_0 = sA_0_0 * sB_0_j1;
            float fused_s_01_1 = sA_0_1 * sB_1_j1;
            float fused_s_10_0 = sA_1_0 * sB_0_j0;
            float fused_s_10_1 = sA_1_1 * sB_1_j0;
            float fused_s_11_0 = sA_1_0 * sB_0_j1;
            float fused_s_11_1 = sA_1_1 * sB_1_j1;

            // Load scaled sums for A
            float sum_a_0_fused_0 = *blockScaledSumA(i, block_idx, block_k / Q_BLK_SIZE);
            float sum_a_0_fused_1 = *blockScaledSumA(i, block_idx + 1, block_k / Q_BLK_SIZE);
            ...
            // Load A data for both i iterations
            __m256i a_pack_int8_low_0 = _mm256_loadu_si256((__m256i *) blockA(i, p, block_k));
            __m256i a_pack_int8_high_0 = _mm256_loadu_si256((__m256i *) blockA(i, p + Q_BLK_SIZE, block_k));
            ...
            extra_sum_fp_00 += sum_a_0_fused_0 * min_b_0_j0;
            extra_sum_fp_00 += sum_a_0_fused_1 * min_b_1_j0;
            ...
            extra_sum_fp_01 += sum_a_0_fused_0 * min_b_0_j1;
            extra_sum_fp_01 += sum_a_0_fused_1 * min_b_1_j1;
            ...
            extra_sum_fp_10 += sum_a_1_fused_0 * min_b_0_j0;
            extra_sum_fp_10 += sum_a_1_fused_1 * min_b_1_j0;
            ...
            extra_sum_fp_11 += sum_a_1_fused_0 * min_b_0_j1;
            extra_sum_fp_11 += sum_a_1_fused_1 * min_b_1_j1;
        }
        
        // Store results
        *C(i, j, NDIM) += (hsum_float_8(sum_packed_fp_00) + extra_sum_fp_00);
            *C(i, j+1, NDIM) += (hsum_float_8(sum_packed_fp_01) + extra_sum_fp_01);
        
            *C(i+1, j, NDIM) += (hsum_float_8(sum_packed_fp_10) + extra_sum_fp_10);
