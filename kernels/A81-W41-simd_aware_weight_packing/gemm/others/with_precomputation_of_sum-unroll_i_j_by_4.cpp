    ...
    // Process 4 rows and 4 columns at a time
        ...
        for (int i = 0; i < block_m; i += 4) 
        {
            ...
            for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE) 
            {
                ...
                for (int ii = 0; ii < 4; ++ii) {
                        sA[ii][0] = *blockSA(i+ii, block_idx, block_k/Q_BLK_SIZE);
                        sA[ii][1] = *blockSA(i+ii, block_idx+1, block_k/Q_BLK_SIZE);
                }
                ...
                for (int ii = 0; ii < 4; ++ii) {
                        a_pack_int8[ii][0] = _mm256_loadu_si256((__m256i *) blockA(i+ii, p, block_k));
                        a_pack_int8[ii][1] = _mm256_loadu_si256((__m256i *) blockA(i+ii, p+Q_BLK_SIZE, block_k));
                }
                ...
                for (int ii = 0; ii < 4; ++ii) {
                    ...
                    for (int jj = 0; jj < 4; ++jj) {
                        ...
                        float sum0 = *blockScaledSumA(ii, block_idx, block_k/Q_BLK_SIZE);
                        float sum1 = *blockScaledSumA(ii, block_idx+1, block_k/Q_BLK_SIZE);
                        float min_b_0 = *blockMinB(block_idx, j+jj, block_k / Q_BLK_SIZE);
                        float min_b_1 = *blockMinB(block_idx + 1, j+jj, block_k / Q_BLK_SIZE);
