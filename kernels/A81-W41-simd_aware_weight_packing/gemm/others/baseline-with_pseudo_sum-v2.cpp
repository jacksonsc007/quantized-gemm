    ...
    for (int j = 0; j < block_n; ++j)
    {
        ...
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            ...
            // so ymm only need to load once
                ...
                // __m256 sum_a_0 = _mm256_set1_ps(1.2);
                // __m256 sum_a_1 = _mm256_set1_ps(1.2);
                // __m256 sum_a_0 = mul_sum_i8_pairs_float(a_pack_int8_low, _mm256_set1_epi8(1));
                // __m256 sum_a_1 = mul_sum_i8_pairs_float(a_pack_int8_high, _mm256_set1_epi8(1));
                // __m256 sum_a_0_fused = _mm256_mul_ps(sum_a_0, _mm256_set1_ps(sA_0));
                // __m256 sum_a_1_fused = _mm256_mul_ps(sum_a_1, _mm256_set1_ps(sA_1));
                __m256 sum_a_0_fused = _mm256_set1_ps(1.2);
                __m256 sum_a_1_fused = _mm256_set1_ps(1.2);
                // printf("sum_a_0: %f, sum_a_1: %f\n", sum_a_0, sum_a_1);
                // exit(0);
                ...
                // // extra_sum_packed_fp += min_b_0 * sum_a_0_fused;
                // // extra_sum_packed_fp += min_b_1 * sum_a_1_fused;
                // sum_packed_fp = _mm256_fmadd_ps(_mm256_set1_ps(min_b_0), sum_a_0_fused, sum_packed_fp);
                // sum_packed_fp = _mm256_fmadd_ps(_mm256_set1_ps(min_b_1), sum_a_1_fused, sum_packed_fp);
                
                // 2.
                sum_packed_fp += (tmp_0 * _mm256_set1_ps(fused_s_0)) + _mm256_set1_ps(min_b_0) * sum_a_0_fused;
                sum_packed_fp += (tmp_1 * _mm256_set1_ps(fused_s_1)) + _mm256_set1_ps(min_b_1) * sum_a_1_fused;
                
                // 3.
                // sum_packed_fp += (tmp_0 * _mm256_set1_ps(fused_s_0)) + _mm256_set1_ps(min_b_0) * sum_a_0_fused + (tmp_1 * _mm256_set1_ps(fused_s_1)) + _mm256_set1_ps(min_b_1) * sum_a_1_fused;
                
                // 4.
                // __m256 tmp0 = (tmp_0 * _mm256_set1_ps(fused_s_0));
                // __m256 tmp1 = (tmp_1 * _mm256_set1_ps(fused_s_1));
                // __m256 tmp2 = _mm256_set1_ps(min_b_0) * sum_a_0_fused;
                // __m256 tmp3 = _mm256_set1_ps(min_b_1) * sum_a_1_fused;
                // sum_packed_fp += tmp0 + tmp1 + tmp2 + tmp3;
...
int main() {
    ...
    // init_rand(blockB_packed, K, NR);
        ...
        matmul_kernel(A, B, C, sA, sB, MinB, M, N, K);
        uint64_t end = timer();
