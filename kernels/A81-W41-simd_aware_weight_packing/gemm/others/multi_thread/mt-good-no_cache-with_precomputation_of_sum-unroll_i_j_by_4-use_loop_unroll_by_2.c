            ...
            for (int p = 0; p < K; p += 2 * Q_BLK_SIZE) 
            {
                ...
                for (int ii = 0; ii < 4; ++ii) {
                    for (int jj = 0; jj < 4; jj += 2) {
                        // jj
                        float fused_s0_0 = sA[ii][0] * sB[jj][0];
                        float fused_s1_0 = sA[ii][1] * sB[jj][1];

                        __m256 tmp0_0 = mul_sum_us8_pairs_float(b_pack_int8_low[jj],  a_pack_int8[ii][0]);
                        __m256 tmp1_0 = mul_sum_us8_pairs_float(b_pack_int8_high[jj], a_pack_int8[ii][1]);

                        sum_packed_fp[ii][jj] = _mm256_fmadd_ps(tmp0_0, _mm256_set1_ps(fused_s0_0), sum_packed_fp[ii][jj]);
                        sum_packed_fp[ii][jj] = _mm256_fmadd_ps(tmp1_0, _mm256_set1_ps(fused_s1_0), sum_packed_fp[ii][jj]);
                        float sum0_0 = *ScaledSumA(i+ii, block_idx, K/Q_BLK_SIZE);
                        float sum1_0 = *ScaledSumA(i+ii, block_idx+1, K/Q_BLK_SIZE);
                        float min_b_0_0 = *MinB(block_idx, j+jj, K / Q_BLK_SIZE);
                        float min_b_1_0 = *MinB(block_idx + 1, j+jj, K / Q_BLK_SIZE);
                        extra_sum[ii][jj] += sum0_0 * min_b_0_0;
                        extra_sum[ii][jj] += sum1_0 * min_b_1_0;

                        // jj+1
                        float fused_s0_1 = sA[ii][0] * sB[jj+1][0];
                        float fused_s1_1 = sA[ii][1] * sB[jj+1][1];

                        __m256 tmp0_1 = mul_sum_us8_pairs_float(b_pack_int8_low[jj+1],  a_pack_int8[ii][0]);
                        __m256 tmp1_1 = mul_sum_us8_pairs_float(b_pack_int8_high[jj+1], a_pack_int8[ii][1]);
                        sum_packed_fp[ii][jj+1] = _mm256_fmadd_ps(tmp0_1, _mm256_set1_ps(fused_s0_1), sum_packed_fp[ii][jj+1]);
                        sum_packed_fp[ii][jj+1] = _mm256_fmadd_ps(tmp1_1, _mm256_set1_ps(fused_s1_1), sum_packed_fp[ii][jj+1]);
                        float sum0_1 = *ScaledSumA(i+ii, block_idx, K/Q_BLK_SIZE);
                        float sum1_1 = *ScaledSumA(i+ii, block_idx+1, K/Q_BLK_SIZE);
                        float min_b_0_1 = *MinB(block_idx, j+jj+1, K / Q_BLK_SIZE);
                        float min_b_1_1 = *MinB(block_idx + 1, j+jj+1, K / Q_BLK_SIZE);
                        extra_sum[ii][jj+1] += sum0_1 * min_b_0_1;
                        extra_sum[ii][jj+1] += sum1_1 * min_b_1_1;
                    }
                }
            }
