            ...
            for (int p = 0; p < K; p += 2 * Q_BLK_SIZE) 
            {
                ...
                for (int ii = 0; ii < 4; ++ii) {
                    ...
                    for (int jj = 0; jj < 4; ++jj) {
                        ...
                        b_pack_int8_low  [ jj ] = _mm256_sign_epi8(b_pack_int8_low  [ jj ], a_pack_int8 [ ii ] [ 0 ] );
                        b_pack_int8_high [ jj ] = _mm256_sign_epi8(b_pack_int8_high [ jj ], a_pack_int8 [ ii ] [ 1 ] );
                        __m256 tmp[2] = {
                        ...
                        float sum0 = *ScaledSumA(ii, block_idx, K/Q_BLK_SIZE);
                        float sum1 = *ScaledSumA(ii, block_idx+1, K/Q_BLK_SIZE);
