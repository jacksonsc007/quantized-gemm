    ...
    const int M, const int N, const int K) 
{
    ...
    for (int j = 0; j < N; j++)
    {
        ...
        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            ...
            float min_b_0 = 0.03;
            float min_b_1 = 0.03;
            float sum_a_0_fused = 0.03;
            float sum_a_1_fused = 0.03;
            
                __m256i a_pack_int8_low = _mm256_set1_epi8(p);
                __m256i a_pack_int8_high = _mm256_set1_epi8(p+1);
                __m256i b_pack_int8 = _mm256_set1_epi8(p + 3);
