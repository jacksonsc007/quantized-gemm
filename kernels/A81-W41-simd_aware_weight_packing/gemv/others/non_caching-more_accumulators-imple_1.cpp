    ...
    for (int j = 0; j < N; j++)
    {
        ...
        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            ...
            float sum_a_1_fused = *ScaledSumA(0, block_idx + 1, K / Q_BLK_SIZE);
                ...
                sum_packed_fp_0 = _mm256_fmadd_ps(tmp_0, _mm256_set1_ps(fused_s_0), sum_packed_fp_0);
                sum_packed_fp_1 = _mm256_fmadd_ps(tmp_1, _mm256_set1_ps(fused_s_1), sum_packed_fp_1);
                ...
                extra_sum_fp_0 += sum_a_0_fused * min_b_0;
                extra_sum_fp_1 += sum_a_1_fused * min_b_1;
        }
        *C(0, j, NDIM) +=  hsum_float_8(sum_packed_fp_0) + extra_sum_fp_0;
        *C(0, j, NDIM) +=  hsum_float_8(sum_packed_fp_1) + extra_sum_fp_1;
    }
}
