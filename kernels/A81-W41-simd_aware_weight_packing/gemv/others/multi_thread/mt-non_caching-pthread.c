...
void* matmul_kernel_thread(void* arg) {
    ThreadArgs* args = (ThreadArgs*)arg;
    int8_t* A = args->A;
    uint8_t* B = args->B;
    float* C = args->C;
    float* SA = args->SA;
    float* ScaledSumA = args->ScaledSumA;
    float* SB = args->SB;
    float* MinB = args->MinB;
    const int M = args->M;
    const int N = args->N;
    const int K = args->K;

    // Only handle 1xK * KxN case
    if (M != 1) {
        fprintf(stderr, "Only M=1 supported for now\n");
        return NULL;
    }
    ...
    for (int j = args->start_col; j < args->end_col; j++) {
        ...
        C[j] += hsum_float_8(sum_packed_fp) + extra_sum_fp;
    }
...
int cols_per_thread = N / NTHREADS;
for (int t = 0; t < NTHREADS; t++) {
    thread_args[t].A = A;
    thread_args[t].B = B;
    thread_args[t].C = C;
    thread_args[t].SA = sA;
    thread_args[t].ScaledSumA = scaledSumA;
    thread_args[t].SB = sB;
    thread_args[t].MinB = MinB;
    thread_args[t].M = M;
    thread_args[t].N = N;
    thread_args[t].K = K;
    thread_args[t].start_col = t * cols_per_thread;
    thread_args[t].end_col = (t == NTHREADS - 1) ? N : (t + 1) * cols_per_thread;

    pthread_create(&threads[t], NULL, matmul_kernel_thread, &thread_args[t]);
}
