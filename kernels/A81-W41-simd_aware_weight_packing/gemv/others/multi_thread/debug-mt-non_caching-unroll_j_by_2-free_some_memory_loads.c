    ...
    const int M, const int N, const int K) 
{
    ...
    for (int j = 0; j < N - 1; j += 2)
    {
        ...
        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            ...
            float sA_1 = sA_0;
            float sB_1_j0 = sB_0_j0;
            float sB_1_j1 = sB_0_j1;
            ...
            float min_b_0_j1 = min_b_0_j0; // Reuse min_b_0_j0 for j+1
            float min_b_1_j0 = *MinB(block_idx + 1, j, K/Q_BLK_SIZE);
            float min_b_1_j1 = min_b_1_j0;
            float sum_a_0_fused = *ScaledSumA(0, block_idx, K/Q_BLK_SIZE);
            float sum_a_1_fused = sum_a_0_fused;
