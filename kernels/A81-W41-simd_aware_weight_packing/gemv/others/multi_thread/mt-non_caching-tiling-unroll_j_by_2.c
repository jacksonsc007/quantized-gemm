...
#define TILE_N  32  // Tile size for N dimension
#define TILE_K  256 // Tile size for K dimension, multiple of 2*Q_BLK_SIZE
        ...
        int jLimit = min(jj + TILE_N, N);
            ...
            for (int j = jj; j < jLimit; j += 2) {
                ...
                for (int p = pp; p < pLimit; p += 2 * Q_BLK_SIZE) {
                    ...
                    float sum_a_0_fused_j1 = *ScaledSumA(0, block_idx, K / Q_BLK_SIZE) ;
                    float sum_a_1_fused_j1 = *ScaledSumA(0, block_idx + 1, K / Q_BLK_SIZE) ;
