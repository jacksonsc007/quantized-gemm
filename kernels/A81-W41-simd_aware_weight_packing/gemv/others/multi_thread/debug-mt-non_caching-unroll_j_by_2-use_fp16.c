    ...
    const int M, const int N, const int K) 
{
    ...
    for (int j = 0; j < N - 1; j += 2)
    {
        ...
        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            ...
            float sA_0          = (float) *SA         ( 0            , block_idx    , K/Q_BLK_SIZE ); 
            float sB_0_j0       = (float) *SB         ( block_idx    , j            , K/Q_BLK_SIZE ); 
            float sB_0_j1       = (float) *SB         ( block_idx    , j+1          , K/Q_BLK_SIZE ); 
            float sA_1          = (float) *SA         ( 0            , block_idx + 1, K/Q_BLK_SIZE ); 
            float sB_1_j0       = (float) *SB         ( block_idx + 1, j            , K/Q_BLK_SIZE ); 
            float sB_1_j1       = (float) *SB         ( block_idx + 1, j+1          , K/Q_BLK_SIZE ); 
            // Load min values and scaled sums for both j values
            float min_b_0_j0    = (float) *MinB       ( block_idx    , j            , K/Q_BLK_SIZE ); 
            float min_b_0_j1    = (float) *MinB       ( block_idx    , j+1          , K/Q_BLK_SIZE ); 
            float min_b_1_j0    = (float) *MinB       ( block_idx + 1, j            , K/Q_BLK_SIZE ); 
            float min_b_1_j1    = (float) *MinB       ( block_idx + 1, j+1          , K/Q_BLK_SIZE ); 
            float sum_a_0_fused = (float) *ScaledSumA ( 0            , block_idx    , K/Q_BLK_SIZE ); 
            float sum_a_1_fused = (float) *ScaledSumA ( 0            , block_idx + 1, K/Q_BLK_SIZE ); 
...
void matmul_naive_no_packing(
    ...
    for (int j = 0; j < N; ++j)
    {
        ...
        for (int p = 0; p < K; p += Q_BLK_SIZE * 2)
        {
            ...
            int block_idx = p / Q_BLK_SIZE;
            float s_a_0 = (float) *SA(i, block_idx, KDIM / Q_BLK_SIZE);
            float s_a_1 = (float) *SA(i, block_idx + 1, KDIM / Q_BLK_SIZE);
            float s_b_0 = (float) *SB(block_idx, j, KDIM / Q_BLK_SIZE);
            float s_b_1 = (float) *SB(block_idx + 1, j, KDIM / Q_BLK_SIZE);
                
                
            float min_b_0 = (float) *MinB(block_idx, j, KDIM / Q_BLK_SIZE);
            float min_b_1 = (float) *MinB(block_idx + 1, j, KDIM / Q_BLK_SIZE);
            float sum_a_0 = (float) *ScaledSumA(i, block_idx, KDIM / Q_BLK_SIZE);
            float sum_a_1 = (float) *ScaledSumA(i, block_idx + 1, KDIM / Q_BLK_SIZE);
...
int main() {
    ...
    init_const_fp16(sA, M, K / Q_BLK_SIZE, 2);
    init_const_fp16(sB, K / Q_BLK_SIZE, N, 3);
    init_const_fp16(MinB, K / Q_BLK_SIZE, N, 4);
    init_const_fp16(scaledSumA, K / Q_BLK_SIZE, N, 4);
