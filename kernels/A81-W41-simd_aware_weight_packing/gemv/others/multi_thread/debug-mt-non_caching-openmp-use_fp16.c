...
static float16 sA_packed[MC * KC / Q_BLK_SIZE] __attribute__((aligned(64)));
static uint8_t blockB_packed[KC / 2 * NC] __attribute__((aligned(64)));
static float16 sB_packed[NC * KC / Q_BLK_SIZE] __attribute__((aligned(64)));
    ...
    const int M, const int N, const int K) 
{
    ...
    for (int j = 0; j < N; j++)
    {
        ...
        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            ...
            float sB_0 = (float) ( *SB(block_idx, j, K/Q_BLK_SIZE)     );
            float sA_1 = (float) ( *SA(0, block_idx + 1, K/Q_BLK_SIZE) );
            float sB_1 = (float) ( *SB(block_idx + 1, j, K/Q_BLK_SIZE) );
            // float fused_s_0 = 0.02;
            // float fused_s_1 = 0.02;
            float fused_s_0 = sA_0 * sB_0;
            float fused_s_1 = sA_1 * sB_1;
            
            float min_b_0       = (float) *MinB(block_idx, j, K / Q_BLK_SIZE);
            float min_b_1       = (float) *MinB(block_idx + 1, j, K / Q_BLK_SIZE);
...
int main() {
    ...
    printf("\e[32m[INFO]\e[m using openmp with %d threads\n", NTHREADS);
    // warm up
