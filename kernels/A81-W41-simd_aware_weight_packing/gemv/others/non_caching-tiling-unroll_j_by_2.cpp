    ...
    const int M, const int N, const int K) 
{
    ...
    const int TILE_N = 32;  // Tile size for N dimension
    const int TILE_K = 256; // Tile size for K dimension, multiple of 2*Q_BLK_SIZE

        // Iterate over tiles in K dimension
    for (int jj = 0; jj < N; jj += TILE_N) 
    {
        
        int jLimit = min(jj + TILE_N, N);
        // Iterate over tiles in N dimension
        for (int pp = 0; pp < K; pp += TILE_K) 
        {
            int pLimit = min(pp + TILE_K, K);
            
            // Process each column within the current N-tile
            for (int j = jj; j < jLimit; j += 2) {
                ...
                // Process each block within the current K-tile
                for (int p = pp; p < pLimit; p += 2 * Q_BLK_SIZE) {
                    ...
                    float sum_a_0_fused_j0 = *ScaledSumA(0, block_idx, K / Q_BLK_SIZE);
                    float sum_a_1_fused_j0 = *ScaledSumA(0, block_idx + 1, K / Q_BLK_SIZE);

                    // For j+1
                    float sB_0_j1 = (j + 1 < jLimit) ? *SB(block_idx, j + 1, K/Q_BLK_SIZE) : 0.0f;
                    float sB_1_j1 = (j + 1 < jLimit) ? *SB(block_idx + 1, j + 1, K/Q_BLK_SIZE) : 0.0f;
                    float fused_s_0_j1 = sA_0 * sB_0_j1;
                    float fused_s_1_j1 = sA_1 * sB_1_j1;
                    float min_b_0_j1 = (j + 1 < jLimit) ? *MinB(block_idx, j + 1, K / Q_BLK_SIZE) : 0.0f;
                    float min_b_1_j1 = (j + 1 < jLimit) ? *MinB(block_idx + 1, j + 1, K / Q_BLK_SIZE) : 0.0f;
                    float sum_a_0_fused_j1 = (j + 1 < jLimit) ? *ScaledSumA(0, block_idx, K / Q_BLK_SIZE) : 0.0f;
                    float sum_a_1_fused_j1 = (j + 1 < jLimit) ? *ScaledSumA(0, block_idx + 1, K / Q_BLK_SIZE) : 0.0f;
                    ...
                    extra_sum_fp_0 += sum_a_0_fused_j0 * min_b_0_j0;
                    extra_sum_fp_0 += sum_a_1_fused_j0 * min_b_1_j0;
                    ...
                    // For j+1
                        ...
                        extra_sum_fp_1 += sum_a_0_fused_j1 * min_b_0_j1;
                        extra_sum_fp_1 += sum_a_1_fused_j1 * min_b_1_j1;
                }

                // Accumulate results for these columns
