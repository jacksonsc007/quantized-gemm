    ...
    const int M, const int N, const int K) 
{
    ...
    PRAGMA_OMP_PARALLEL_FOR
        ...
        // for (int i = 0; i < M; i += 4) 
        {
            ...
            __m256 sum_packed_fp[4];
            float extra_sum[4];
            // float sum_a_fused[4][4];
            // float min_b[4][4];
                for (int jj = 0; jj < 4; ++jj) {
                    sum_packed_fp[jj] = _mm256_setzero_ps();
                    extra_sum[jj] = 0;
                    // min_b[ii][jj] = 0.01;
            ...
            for (int p = 0; p < K; p += 2 * Q_BLK_SIZE) 
            {
                ...
                float sA[2]; // [i][0/1 for low/high block]
                float sB[4][2]; // [j][0/1 for low/high block]
                
                sA[0] = *SA(0, block_idx, K/Q_BLK_SIZE);
                sA[1] = *SA(0, block_idx+1, K/Q_BLK_SIZE);
                ...
                __m256i a_pack_int8[2]; // [i][0/1 for low/high block]
                a_pack_int8[0] = _mm256_loadu_si256((__m256i *) A(0, p, K));
                a_pack_int8[1] = _mm256_loadu_si256((__m256i *) A(0, p+Q_BLK_SIZE, K));
                ...
                // Compute all active i/j combinations
                    ...
                    for (int jj = 0; jj < 4; ++jj) {
                        ...
                        __m256 tmp[2] = {
                            mul_sum_us8_pairs_float(b_pack_int8_low  [ jj ], a_pack_int8 [ 0 ] ),
                            mul_sum_us8_pairs_float(b_pack_int8_high [ jj ], a_pack_int8 [ 1 ] )
                        };
                        
                        sum_packed_fp[jj] = _mm256_fmadd_ps(tmp[0], _mm256_set1_ps(fused_s[0]), sum_packed_fp[jj]);
                        sum_packed_fp[jj] = _mm256_fmadd_ps(tmp[1], _mm256_set1_ps(fused_s[1]), sum_packed_fp[jj]);
                    
                        float sum0 = *ScaledSumA(0, block_idx, K/Q_BLK_SIZE);
                        float sum1 = *ScaledSumA(0, block_idx+1, K/Q_BLK_SIZE);
                        float min_b_0 = *MinB(block_idx, j+jj, K / Q_BLK_SIZE);
                        float min_b_1 = *MinB(block_idx + 1, j+jj, K / Q_BLK_SIZE);
                        extra_sum[jj] += sum0 * min_b_0;
                        extra_sum[jj] += sum1 * min_b_1;
                    }
            ...
            // Store results for all active i/j combinations
                ...
                for (int jj = 0; jj < 4; ++jj) {
                    ...
                    *C(0, j+jj, NDIM) += hsum_float_8(sum_packed_fp[jj]) + extra_sum[jj];
                }
        }
}
