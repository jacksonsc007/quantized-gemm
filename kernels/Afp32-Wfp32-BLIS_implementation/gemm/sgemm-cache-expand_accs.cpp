...
void microkernel_16x6(float *blockA, float *blockB, float *C, const int valid_m,
  ...
  if (valid_n != NR) {
    ...
    mask0 = _mm256_setr_epi32(
        bit_mask << (valid_n + 15), bit_mask << (valid_n + 14),
        bit_mask << (valid_n + 13), bit_mask << (valid_n + 12),
        bit_mask << (valid_n + 11), bit_mask << (valid_n + 10),
        bit_mask << (valid_n + 9), bit_mask << (valid_n + 8));
    mask1 =
        _mm256_setr_epi32(bit_mask << (valid_n + 7), bit_mask << (valid_n + 6),
                          bit_mask << (valid_n + 5), bit_mask << (valid_n + 4),
                          bit_mask << (valid_n + 3), bit_mask << (valid_n + 2),
                          bit_mask << (valid_n + 1), bit_mask << valid_n);
    switch (valid_m) {
    case 6:
      ...
      for (int ic = 0; ic < M; ic += MC) {
        ...
        for (int jr = 0; jr < valid_NC; jr += NR) {
          ...
          for (int ir = 0; ir < valid_MC; ir += MR) {
            ...
            // printf("C[%d, %d] + [%d, %d]\n", ic + ir, jc + jr, valid_MR,
            // valid_NR);
            microkernel_16x6(blockA_panel, blockB_panel, C(ic + ir, jc + jr),
                             valid_MR, valid_NR, valid_MR, valid_NR, valid_KC,
                             N);
            // kernel_16x6(blockA_panel, blockB_panel, C(ic + ir, jc + jr),
            // valid_MR, valid_NR, valid_MR, valid_NR, valid_KC, N, ic+ir,
            // jc+jr, C); kernel_16x6(blockA_panel, blockB_panel, C((ic + ir),
            // (jc + jr)), valid_MR, valid_NR, valid_MR, valid_NR, valid_KC, N,
            // ic+ir, jc+jr, C); kernel_16x6(blockA_panel, blockB_panel, C((ic +
            // ir), (jc + jr)), valid_MR, valid_NR, valid_MR, valid_NR,
            // valid_KC, N, ic+ir, jc+jr, C); kernel_16x6(blockA_panel,
            // blockB_panel, C((ic + ir), (jc + jr)), valid_MR, valid_NR, MR,
            // NR, valid_KC, N, ic+ir, jc+jr, C);
          }
        }
      }
    }
