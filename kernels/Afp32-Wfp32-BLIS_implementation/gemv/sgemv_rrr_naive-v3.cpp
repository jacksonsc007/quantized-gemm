...
void kernel_per_thread(float* A, float* B, float* C, const int N, const int K, const int start_j, const int end_j) {
    for (int p = 0; p < K; p++)
    {
...
void matmul_naive(float* A, float* B, float* C, const int N, const int K)
{
    #pragma omp parallel num_threads(NTHREADS)
    {
        ...
        {
            const int start_j = j;
            const int end_j = min(j+SIZE, N);
            kernel_per_thread(A, B, C, N, K, start_j, end_j);
        }
    }
}
