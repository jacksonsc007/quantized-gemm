...
static inline float sum_uint8_i32(const __m256i x) {
...
void init_rand(float* mat, const int M, const int N, const float multiplier) {
    for (int i = 0; i < M * N; i++) {
        *mat++ = rand() / (float)RAND_MAX * multiplier;
    }
}
...
void init_rand_uint8(int8_t* mat, const int M, const int N) {
...
int compare_mats(float* mat1, float* mat2, const int32_t size) {
    int num = 1200;
    float acc_err = 0.;
    // for (int i = 0; i < M; i++) 
    ...
    for (int i = 0; i < size; i++) 
    {
        float err = fabsf(mat1[i] - mat2[i]);
        acc_err += err;
    }

    float avg_err = acc_err / size;
