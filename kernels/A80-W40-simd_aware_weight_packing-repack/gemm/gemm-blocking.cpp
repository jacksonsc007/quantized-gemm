...
)
{
    ...
    const int BLOCK_N = 8;   // Process 8 columns at a time

    for (int i = 0; i < nb_m; i += BLOCK_M)
    {
        int block_m = min(BLOCK_M, nb_m - i);
        for(int j = 0; j < nb_n; j += BLOCK_N)
        {
            int block_n = min(BLOCK_N, nb_n - j);
            
            // Process each block
            for (int ii = 0; ii < block_m; ii++)
            {
                struct q80_repack_1x2 * A_ptr = A_ptr_start + (i + ii) * nb_k;
                __m256 acc_rows[BLOCK_N];
                for (int jj = 0; jj < block_n; jj++)
                {
                    acc_rows[jj] = _mm256_setzero_ps();
                }

                for (int blk_id = 0; blk_id < nb_k; blk_id++)
                {
                    // Pre-load A block data
                    ...
                    for (int jj = 0; jj < block_n; jj++)
                    {
                        struct q40_repack_2x8 B_block = B_ptr_start[(j + jj) * nb_k + blk_id];
                        
                        // Process B block data
                        ...
                        for (int pack_idx = 0; pack_idx < 8; pack_idx++)
                        {
                            ...
                            low_blk_B_packs[pack_idx] = _mm256_sub_epi8(
                                _mm256_and_si256(B_coupled_pack, _mm256_set1_epi8(0x0f)),
                                _mm256_set1_epi8(8)
                            );
                            high_blk_B_packs[pack_idx] = _mm256_sub_epi8(
                                _mm256_and_si256(_mm256_srli_epi16(B_coupled_pack, 4), _mm256_set1_epi8(0x0f)),
                                _mm256_set1_epi8(8)
                            );
                        }
                        ...
                        for (int pack_idx = 0; pack_idx < 8; pack_idx += 2)
                        {
                            ...
                            iacc_low = mul_sum_i8_pairs_acc_int32x8(iacc_low, low_blk_A_packs[pack_idx + 1], low_blk_B_packs[pack_idx + 1]);
                            iacc_high = mul_sum_i8_pairs_acc_int32x8(iacc_high, high_blk_A_packs[pack_idx + 1], high_blk_B_packs[pack_idx + 1]);
                        }
                        ...
                        // Accumulate results
                        acc_rows[jj] = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low), s_low, acc_rows[jj]);
                        acc_rows[jj] = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high), s_high, acc_rows[jj]);
                    }
                }
                ...
                for (int jj = 0; jj < block_n; jj++)
                {
                    _mm256_storeu_ps(C(i + ii, (j + jj) * 8, N), acc_rows[jj]);
                }
            }
        }
    }
