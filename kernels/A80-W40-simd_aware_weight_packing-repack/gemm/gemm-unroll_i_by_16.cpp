...
)
{
    ...
    for (int i = 0; i < nb_m; i += 16)
    {
        for(int j = 0; j < nb_n; j++)
        {
            ...
            struct q80_repack_1x2 * A_ptr8 = A_ptr_start + (i + 8) * nb_k;
            struct q80_repack_1x2 * A_ptr9 = A_ptr_start + (i + 9) * nb_k;
            struct q80_repack_1x2 * A_ptr10 = A_ptr_start + (i + 10) * nb_k;
            struct q80_repack_1x2 * A_ptr11 = A_ptr_start + (i + 11) * nb_k;
            struct q80_repack_1x2 * A_ptr12 = A_ptr_start + (i + 12) * nb_k;
            struct q80_repack_1x2 * A_ptr13 = A_ptr_start + (i + 13) * nb_k;
            struct q80_repack_1x2 * A_ptr14 = A_ptr_start + (i + 14) * nb_k;
            struct q80_repack_1x2 * A_ptr15 = A_ptr_start + (i + 15) * nb_k;
            ...
            __m256 acc_row8 = _mm256_setzero_ps();
            __m256 acc_row9 = _mm256_setzero_ps();
            __m256 acc_row10 = _mm256_setzero_ps();
            __m256 acc_row11 = _mm256_setzero_ps();
            __m256 acc_row12 = _mm256_setzero_ps();
            __m256 acc_row13 = _mm256_setzero_ps();
            __m256 acc_row14 = _mm256_setzero_ps();
            __m256 acc_row15 = _mm256_setzero_ps();
            for (int blk_id = 0; blk_id < nb_k; blk_id++)
            {
                ...
                // Process ninth row
                __m256i low_blk_A8_packs[8];
                __m256i high_blk_A8_packs[8];
                __m256i low_blk_A8 = _mm256_loadu_si256((__m256i *) (A_ptr8[blk_id].q_low));
                __m256i high_blk_A8 = _mm256_loadu_si256((__m256i *) (A_ptr8[blk_id].q_high));
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A8_packs[pack_idx] = _mm256_permutevar8x32_epi32(low_blk_A8, perms[pack_idx]);
                    high_blk_A8_packs[pack_idx] = _mm256_permutevar8x32_epi32(high_blk_A8, perms[pack_idx]);
                }
                
                __m256i iacc_low8 = _mm256_setzero_si256();
                __m256i iacc_high8 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low8 = mul_sum_i8_pairs_acc_int32x8(iacc_low8, low_blk_A8_packs[pack_idx], low_blk_B_packs[pack_idx]);
                    iacc_high8 = mul_sum_i8_pairs_acc_int32x8(iacc_high8, high_blk_A8_packs[pack_idx], high_blk_B_packs[pack_idx]);
                }

                // Process tenth row
                __m256i low_blk_A9_packs[8];
                __m256i high_blk_A9_packs[8];
                __m256i low_blk_A9 = _mm256_loadu_si256((__m256i *) (A_ptr9[blk_id].q_low));
                __m256i high_blk_A9 = _mm256_loadu_si256((__m256i *) (A_ptr9[blk_id].q_high));
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A9_packs[pack_idx] = _mm256_permutevar8x32_epi32(low_blk_A9, perms[pack_idx]);
                    high_blk_A9_packs[pack_idx] = _mm256_permutevar8x32_epi32(high_blk_A9, perms[pack_idx]);
                }
                
                __m256i iacc_low9 = _mm256_setzero_si256();
                __m256i iacc_high9 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low9 = mul_sum_i8_pairs_acc_int32x8(iacc_low9, low_blk_A9_packs[pack_idx], low_blk_B_packs[pack_idx]);
                    iacc_high9 = mul_sum_i8_pairs_acc_int32x8(iacc_high9, high_blk_A9_packs[pack_idx], high_blk_B_packs[pack_idx]);
                }

                // Process eleventh row
                __m256i low_blk_A10_packs[8];
                __m256i high_blk_A10_packs[8];
                __m256i low_blk_A10 = _mm256_loadu_si256((__m256i *) (A_ptr10[blk_id].q_low));
                __m256i high_blk_A10 = _mm256_loadu_si256((__m256i *) (A_ptr10[blk_id].q_high));
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A10_packs[pack_idx] = _mm256_permutevar8x32_epi32(low_blk_A10, perms[pack_idx]);
                    high_blk_A10_packs[pack_idx] = _mm256_permutevar8x32_epi32(high_blk_A10, perms[pack_idx]);
                }
                
                __m256i iacc_low10 = _mm256_setzero_si256();
                __m256i iacc_high10 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low10 = mul_sum_i8_pairs_acc_int32x8(iacc_low10, low_blk_A10_packs[pack_idx], low_blk_B_packs[pack_idx]);
                    iacc_high10 = mul_sum_i8_pairs_acc_int32x8(iacc_high10, high_blk_A10_packs[pack_idx], high_blk_B_packs[pack_idx]);
                }

                // Process twelfth row
                __m256i low_blk_A11_packs[8];
                __m256i high_blk_A11_packs[8];
                __m256i low_blk_A11 = _mm256_loadu_si256((__m256i *) (A_ptr11[blk_id].q_low));
                __m256i high_blk_A11 = _mm256_loadu_si256((__m256i *) (A_ptr11[blk_id].q_high));
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A11_packs[pack_idx] = _mm256_permutevar8x32_epi32(low_blk_A11, perms[pack_idx]);
                    high_blk_A11_packs[pack_idx] = _mm256_permutevar8x32_epi32(high_blk_A11, perms[pack_idx]);
                }
                
                __m256i iacc_low11 = _mm256_setzero_si256();
                __m256i iacc_high11 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low11 = mul_sum_i8_pairs_acc_int32x8(iacc_low11, low_blk_A11_packs[pack_idx], low_blk_B_packs[pack_idx]);
                    iacc_high11 = mul_sum_i8_pairs_acc_int32x8(iacc_high11, high_blk_A11_packs[pack_idx], high_blk_B_packs[pack_idx]);
                }

                // Process thirteenth row
                __m256i low_blk_A12_packs[8];
                __m256i high_blk_A12_packs[8];
                __m256i low_blk_A12 = _mm256_loadu_si256((__m256i *) (A_ptr12[blk_id].q_low));
                __m256i high_blk_A12 = _mm256_loadu_si256((__m256i *) (A_ptr12[blk_id].q_high));
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A12_packs[pack_idx] = _mm256_permutevar8x32_epi32(low_blk_A12, perms[pack_idx]);
                    high_blk_A12_packs[pack_idx] = _mm256_permutevar8x32_epi32(high_blk_A12, perms[pack_idx]);
                }
                
                __m256i iacc_low12 = _mm256_setzero_si256();
                __m256i iacc_high12 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low12 = mul_sum_i8_pairs_acc_int32x8(iacc_low12, low_blk_A12_packs[pack_idx], low_blk_B_packs[pack_idx]);
                    iacc_high12 = mul_sum_i8_pairs_acc_int32x8(iacc_high12, high_blk_A12_packs[pack_idx], high_blk_B_packs[pack_idx]);
                }

                // Process fourteenth row
                __m256i low_blk_A13_packs[8];
                __m256i high_blk_A13_packs[8];
                __m256i low_blk_A13 = _mm256_loadu_si256((__m256i *) (A_ptr13[blk_id].q_low));
                __m256i high_blk_A13 = _mm256_loadu_si256((__m256i *) (A_ptr13[blk_id].q_high));
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A13_packs[pack_idx] = _mm256_permutevar8x32_epi32(low_blk_A13, perms[pack_idx]);
                    high_blk_A13_packs[pack_idx] = _mm256_permutevar8x32_epi32(high_blk_A13, perms[pack_idx]);
                }
                
                __m256i iacc_low13 = _mm256_setzero_si256();
                __m256i iacc_high13 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low13 = mul_sum_i8_pairs_acc_int32x8(iacc_low13, low_blk_A13_packs[pack_idx], low_blk_B_packs[pack_idx]);
                    iacc_high13 = mul_sum_i8_pairs_acc_int32x8(iacc_high13, high_blk_A13_packs[pack_idx], high_blk_B_packs[pack_idx]);
                }

                // Process fifteenth row
                __m256i low_blk_A14_packs[8];
                __m256i high_blk_A14_packs[8];
                __m256i low_blk_A14 = _mm256_loadu_si256((__m256i *) (A_ptr14[blk_id].q_low));
                __m256i high_blk_A14 = _mm256_loadu_si256((__m256i *) (A_ptr14[blk_id].q_high));
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A14_packs[pack_idx] = _mm256_permutevar8x32_epi32(low_blk_A14, perms[pack_idx]);
                    high_blk_A14_packs[pack_idx] = _mm256_permutevar8x32_epi32(high_blk_A14, perms[pack_idx]);
                }
                
                __m256i iacc_low14 = _mm256_setzero_si256();
                __m256i iacc_high14 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low14 = mul_sum_i8_pairs_acc_int32x8(iacc_low14, low_blk_A14_packs[pack_idx], low_blk_B_packs[pack_idx]);
                    iacc_high14 = mul_sum_i8_pairs_acc_int32x8(iacc_high14, high_blk_A14_packs[pack_idx], high_blk_B_packs[pack_idx]);
                }

                // Process sixteenth row
                __m256i low_blk_A15_packs[8];
                __m256i high_blk_A15_packs[8];
                __m256i low_blk_A15 = _mm256_loadu_si256((__m256i *) (A_ptr15[blk_id].q_low));
                __m256i high_blk_A15 = _mm256_loadu_si256((__m256i *) (A_ptr15[blk_id].q_high));
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A15_packs[pack_idx] = _mm256_permutevar8x32_epi32(low_blk_A15, perms[pack_idx]);
                    high_blk_A15_packs[pack_idx] = _mm256_permutevar8x32_epi32(high_blk_A15, perms[pack_idx]);
                }
                
                __m256i iacc_low15 = _mm256_setzero_si256();
                __m256i iacc_high15 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low15 = mul_sum_i8_pairs_acc_int32x8(iacc_low15, low_blk_A15_packs[pack_idx], low_blk_B_packs[pack_idx]);
                    iacc_high15 = mul_sum_i8_pairs_acc_int32x8(iacc_high15, high_blk_A15_packs[pack_idx], high_blk_B_packs[pack_idx]);
                }

                // Process scaling for rows 8-15
                // Process ninth row scaling
                __m256 sA_low8 = _mm256_set1_ps(A_ptr8[blk_id].s_low);
                __m256 sA_high8 = _mm256_set1_ps(A_ptr8[blk_id].s_high);
                __m256 s_low8 = _mm256_mul_ps(sA_low8, sB_low);
                __m256 s_high8 = _mm256_mul_ps(sA_high8, sB_high);
                acc_row8 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low8), s_low8, acc_row8);
                acc_row8 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high8), s_high8, acc_row8);

                // Process tenth row scaling
                __m256 sA_low9 = _mm256_set1_ps(A_ptr9[blk_id].s_low);
                __m256 sA_high9 = _mm256_set1_ps(A_ptr9[blk_id].s_high);
                __m256 s_low9 = _mm256_mul_ps(sA_low9, sB_low);
                __m256 s_high9 = _mm256_mul_ps(sA_high9, sB_high);
                acc_row9 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low9), s_low9, acc_row9);
                acc_row9 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high9), s_high9, acc_row9);

                // Process eleventh row scaling
                __m256 sA_low10 = _mm256_set1_ps(A_ptr10[blk_id].s_low);
                __m256 sA_high10 = _mm256_set1_ps(A_ptr10[blk_id].s_high);
                __m256 s_low10 = _mm256_mul_ps(sA_low10, sB_low);
                __m256 s_high10 = _mm256_mul_ps(sA_high10, sB_high);
                acc_row10 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low10), s_low10, acc_row10);
                acc_row10 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high10), s_high10, acc_row10);

                // Process twelfth row scaling
                __m256 sA_low11 = _mm256_set1_ps(A_ptr11[blk_id].s_low);
                __m256 sA_high11 = _mm256_set1_ps(A_ptr11[blk_id].s_high);
                __m256 s_low11 = _mm256_mul_ps(sA_low11, sB_low);
                __m256 s_high11 = _mm256_mul_ps(sA_high11, sB_high);
                acc_row11 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low11), s_low11, acc_row11);
                acc_row11 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high11), s_high11, acc_row11);

                // Process thirteenth row scaling
                __m256 sA_low12 = _mm256_set1_ps(A_ptr12[blk_id].s_low);
                __m256 sA_high12 = _mm256_set1_ps(A_ptr12[blk_id].s_high);
                __m256 s_low12 = _mm256_mul_ps(sA_low12, sB_low);
                __m256 s_high12 = _mm256_mul_ps(sA_high12, sB_high);
                acc_row12 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low12), s_low12, acc_row12);
                acc_row12 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high12), s_high12, acc_row12);

                // Process fourteenth row scaling
                __m256 sA_low13 = _mm256_set1_ps(A_ptr13[blk_id].s_low);
                __m256 sA_high13 = _mm256_set1_ps(A_ptr13[blk_id].s_high);
                __m256 s_low13 = _mm256_mul_ps(sA_low13, sB_low);
                __m256 s_high13 = _mm256_mul_ps(sA_high13, sB_high);
                acc_row13 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low13), s_low13, acc_row13);
                acc_row13 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high13), s_high13, acc_row13);

                // Process fifteenth row scaling
                __m256 sA_low14 = _mm256_set1_ps(A_ptr14[blk_id].s_low);
                __m256 sA_high14 = _mm256_set1_ps(A_ptr14[blk_id].s_high);
                __m256 s_low14 = _mm256_mul_ps(sA_low14, sB_low);
                __m256 s_high14 = _mm256_mul_ps(sA_high14, sB_high);
                acc_row14 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low14), s_low14, acc_row14);
                acc_row14 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high14), s_high14, acc_row14);

                // Process sixteenth row scaling
                __m256 sA_low15 = _mm256_set1_ps(A_ptr15[blk_id].s_low);
                __m256 sA_high15 = _mm256_set1_ps(A_ptr15[blk_id].s_high);
                __m256 s_low15 = _mm256_mul_ps(sA_low15, sB_low);
                __m256 s_high15 = _mm256_mul_ps(sA_high15, sB_high);
                acc_row15 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low15), s_low15, acc_row15);
                acc_row15 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high15), s_high15, acc_row15);
            }
            ...
            _mm256_storeu_ps(C(i + 8, j * 8, N), acc_row8);
            _mm256_storeu_ps(C(i + 9, j * 8, N), acc_row9);
            _mm256_storeu_ps(C(i + 10, j * 8, N), acc_row10);
            _mm256_storeu_ps(C(i + 11, j * 8, N), acc_row11);
            _mm256_storeu_ps(C(i + 12, j * 8, N), acc_row12);
            _mm256_storeu_ps(C(i + 13, j * 8, N), acc_row13);
            _mm256_storeu_ps(C(i + 14, j * 8, N), acc_row14);
            _mm256_storeu_ps(C(i + 15, j * 8, N), acc_row15);
        }
    }
}
