    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    ...
    for (int j = 0; j < block_n; ++j)
    {
        __m256 sum_packed_fp_i0 = _mm256_setzero_ps();
        __m256 sum_packed_fp_i1 = _mm256_setzero_ps();
        
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            int block_idx = p / Q_BLK_SIZE;
            
            // Load scaling factors for i and i+1
            float sA_0_i0 = *blockSA(i, block_idx, block_k/Q_BLK_SIZE);
            float sA_1_i0 = *blockSA(i, block_idx + 1, block_k/Q_BLK_SIZE);
            float sB_0 = *blockSB(block_idx, j, block_k/Q_BLK_SIZE);
            float sB_1 = *blockSB(block_idx + 1, j, block_k/Q_BLK_SIZE);
            
            float fused_s_0_i0 = sA_0_i0 * sB_0;
            float fused_s_1_i0 = sA_1_i0 * sB_1;
            
            bool process_i1 = (i + 1) < block_m;
            float fused_s_0_i1 = 0, fused_s_1_i1 = 0;
            if (process_i1) {
                float sA_0_i1 = *blockSA(i+1, block_idx, block_k/Q_BLK_SIZE);
                float sA_1_i1 = *blockSA(i+1, block_idx + 1, block_k/Q_BLK_SIZE);
                fused_s_0_i1 = sA_0_i1 * sB_0;
                fused_s_1_i1 = sA_1_i1 * sB_1;
            }

            // Load A blocks for i and i+1
            __m256i a_pack_int8_low_i0 = _mm256_loadu_si256((__m256i *) blockA(i, p, block_k));
            __m256i a_pack_int8_high_i0 = _mm256_loadu_si256((__m256i *) blockA(i, p + Q_BLK_SIZE, block_k));
            
            __m256i a_pack_int8_low_i1, a_pack_int8_high_i1;
            if (process_i1) {
                a_pack_int8_low_i1 = _mm256_loadu_si256((__m256i *) blockA(i+1, p, block_k));
                a_pack_int8_high_i1 = _mm256_loadu_si256((__m256i *) blockA(i+1, p + Q_BLK_SIZE, block_k));
            }

            // Load B blocks (same for both i iterations)
            ...
            // Compute for i
            __m256 tmp_0_i0 = mul_sum_i8_pairs_float(a_pack_int8_low_i0, b_pack_int8_low);
            __m256 tmp_1_i0 = mul_sum_i8_pairs_float(a_pack_int8_high_i0, b_pack_int8_high);
            sum_packed_fp_i0 = _mm256_fmadd_ps(tmp_0_i0, _mm256_set1_ps(fused_s_0_i0), sum_packed_fp_i0);
            sum_packed_fp_i0 = _mm256_fmadd_ps(tmp_1_i0, _mm256_set1_ps(fused_s_1_i0), sum_packed_fp_i0);
            
            // Compute for i+1 if needed
            if (process_i1) {
                __m256 tmp_0_i1 = mul_sum_i8_pairs_float(a_pack_int8_low_i1, b_pack_int8_low);
                __m256 tmp_1_i1 = mul_sum_i8_pairs_float(a_pack_int8_high_i1, b_pack_int8_high);
                sum_packed_fp_i1 = _mm256_fmadd_ps(tmp_0_i1, _mm256_set1_ps(fused_s_0_i1), sum_packed_fp_i1);
                sum_packed_fp_i1 = _mm256_fmadd_ps(tmp_1_i1, _mm256_set1_ps(fused_s_1_i1), sum_packed_fp_i1);
            }
        }

        // Store results for i
        *C(i, j, NDIM) += hsum_float_8(sum_packed_fp_i0);
        
        // Store results for i+1 if needed
        if ((i + 1) < block_m) {
            *C(i+1, j, NDIM) += hsum_float_8(sum_packed_fp_i1);
        }
    }
}
