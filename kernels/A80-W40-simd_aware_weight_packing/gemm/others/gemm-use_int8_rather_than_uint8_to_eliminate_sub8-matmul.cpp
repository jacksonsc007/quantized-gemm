...
static int8_t blockB_packed[KC / 2 * NC] __attribute__((aligned(64)));
    ...
    for (int j = 0; j < block_n; ++j)
    {
        ...
        for (int p = 0; p < block_k; p += Q_BLK_SIZE * 2)
        {
            ...
            for (int q = 0; q < Q_BLK_SIZE; q++)
            {
                ...
                int8_t b = *blockB(p / 2 + q, j, block_k / 2);
...
int main() {
    ...
    int8_t* B = (int8_t*)_mm_malloc(K / 2 * N * sizeof(int8_t), MEM_ALIGN);
