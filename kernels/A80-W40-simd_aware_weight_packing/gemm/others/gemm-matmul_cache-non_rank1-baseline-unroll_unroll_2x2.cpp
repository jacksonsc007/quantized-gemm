    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
        ...
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            ...
            float sB_0_j0 = *blockSB(block_idx, 0, block_k/Q_BLK_SIZE);
            float sB_1_j0 = *blockSB(block_idx + 1, 0, block_k/Q_BLK_SIZE);
            float sB_0_j1 = *blockSB(block_idx, 0+1, block_k/Q_BLK_SIZE);
            float sB_1_j1 = *blockSB(block_idx + 1, 0+1, block_k/Q_BLK_SIZE);
            ...
            __m256i b_pack_int8_j0 = _mm256_loadu_si256((__m256i *) blockB(p/2, 0, block_k/2));
            __m256i b_pack_int8_j1 = _mm256_loadu_si256((__m256i *) blockB(p/2, 0+1, block_k/2));
        ...
        // Store results for j
        *C(0, 0, NDIM) += hsum_float_8(sum_packed_fp_i0_j0);
            *C(1, 0, NDIM) += hsum_float_8(sum_packed_fp_i1_j0);
        
        // Store results for j+1 if needed
            *C(0, 0+1, NDIM) += hsum_float_8(sum_packed_fp_i0_j1);
                *C(1, 0+1, NDIM) += hsum_float_8(sum_packed_fp_i1_j1);
}
