    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    ...
    for (int j = 0; j < block_n; ++j)
    {
        ...
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            ...
            bool process_i1 = (0 + 1) < block_m;
            float fused_s_0_i1 = 0, fused_s_1_i1 = 0;
                float sA_0_i1 = *blockSA(0+1, block_idx, block_k/Q_BLK_SIZE);
                float sA_1_i1 = *blockSA(0+1, block_idx + 1, block_k/Q_BLK_SIZE);
            ...
            __m256i a_pack_int8_low_i1, a_pack_int8_high_i1;
                a_pack_int8_low_i1 = _mm256_loadu_si256((__m256i *) blockA(0+1, p, block_k));
                a_pack_int8_high_i1 = _mm256_loadu_si256((__m256i *) blockA(0+1, p + Q_BLK_SIZE, block_k));
        ...
        // Store results for i
        *C(0, j, NDIM) += hsum_float_8(sum_packed_fp_i0);
        
        // Store results for 0+1 if needed
        *C(0+1, j, NDIM) += hsum_float_8(sum_packed_fp_i1);
    }
}
