    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    ...
    for (int j = 0; j < block_n; ++j)
    {
        ...
        for (int p = 0; p < block_k; p += Q_BLK_SIZE * 2)
        {
            ...
            float s_a_0 = *blockSA(i, block_idx, KDIM / Q_BLK_SIZE);
            float s_b_0 = *blockSB(block_idx, j, KDIM / Q_BLK_SIZE);
            float s_a_1 = *blockSA(i, block_idx + 1, KDIM / Q_BLK_SIZE);
            float s_b_1 = *blockSB(block_idx + 1, j, KDIM / Q_BLK_SIZE);
            for (int q = 0; q < Q_BLK_SIZE; q++)
            {
                int8_t a_low = *blockA(i, p + q, KDIM);
                int8_t a_high = *blockA(i, p + q + Q_BLK_SIZE, KDIM); 
                uint8_t b = *blockB(p / 2 + q, j, KDIM / 2);
    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    ...
    for (int j = 0; j < block_n; ++j)
    {
        ...
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            ...
            float sB_0 = *blockSB(block_idx, j, KDIM/Q_BLK_SIZE);
            float sA_1 = *blockSA(0, block_idx + 1, KDIM/Q_BLK_SIZE);
            float sB_1 = *blockSB(block_idx + 1, j, KDIM/Q_BLK_SIZE);
            ...
            float fused_s_1 = sA_1 * sB_1;
                ...
                __m256i b_pack_int8 = _mm256_loadu_si256( (__m256i *) blockB(p/2, j, KDIM/2));
...
// Forward declaration for float_matmul_naive
