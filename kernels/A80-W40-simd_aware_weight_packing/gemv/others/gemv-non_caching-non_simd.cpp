    ...
    const int M, const int N, const int K) 
{
    ...
    for (int j = 0; j < N; ++j)
    {
        ...
        for (int p = 0; p < K; p += Q_BLK_SIZE * 2)
        {
            ...
            float s_a_0 = *SA(0, block_idx, K / Q_BLK_SIZE);
            float s_b_0 = *SB(block_idx, j, K / Q_BLK_SIZE);
            float s_a_1 = *SA(0, block_idx + 1, K / Q_BLK_SIZE);
            float s_b_1 = *SB(block_idx + 1, j, K / Q_BLK_SIZE);
            for (int q = 0; q < Q_BLK_SIZE; q++)
            {
                int8_t a_low = *A(0, p + q, K);
                int8_t a_high = *A(0, p + q + Q_BLK_SIZE, K); 
                uint8_t b = *B(p / 2 + q, j, K / 2);
