...
static void * matmul_kernel_worker_func( void * args) 
{
    struct w4a8_thread_args *thread_args = (struct w4a8_thread_args *) args;    
    const struct matmul_params *params = thread_args->params;
    int start_j = thread_args->start_j;
    int end_j = thread_args->end_j;
    int8_t* A = params->A;
    uint8_t* B = params->B;
    float* C = params->C;
    float* SA = params->SA;
...
void matmul_kernel(
    ...
    struct matmul_params params = {A, B, C, SA, SB, M, N, K};
    struct w4a8_thread_args thread_args[num_threads];
    for (int i = 0; i < num_threads; i++) {
        thread_args[i].start_j = i * block_size;
        thread_args[i].end_j = min(thread_args[i].start_j + block_size, N);
        thread_args[i].params = &params;
        pthread_create(&threads[i], NULL, matmul_kernel_worker_func, &thread_args[i]);
    }
    for (int i = 0; i < num_threads; i++) {
...
int main() {
    ...
    printf("\n\e[32m[INFO]\e[m Using pthread. NTHREADS=%d\n", NTHREADS);
