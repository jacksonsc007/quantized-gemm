...
Several changes are made to the original code:
1. No need to pack A
2. Added multithreading support
*/
#include <assert.h>
#include <immintrin.h>
#include <math.h>
...
void* matmul_kernel_thread(void* arg) {
    ...
    // Divide N dimension among threads
    int chunk_size = (N + data->num_threads - 1) / data->num_threads;
    int start_j = data->thread_id * chunk_size;
    int end_j = min(start_j + chunk_size, N);
...
void matmul_kernel_multithreaded(
    ...
    for (int i = 0; i < NTHREADS; i++) {
        ...
        thread_data[i].num_threads = NTHREADS;
    }
    
    // Create threads
    for (int i = 0; i < NTHREADS; i++) {
        pthread_create(&threads[i], NULL, matmul_kernel_thread, &thread_data[i]);
    }
...
int main() {
    ...
    printf("\n\e[32m[INFO]\e[m MR=%d, NR=%d, MC=%d, NC=%d, KC=%d, M=%d, K=%d, N=%d, NTHREADS=%d\n", 
           MR, NR, MC, NC, KC, M, K, N, NTHREADS);
    ...
    for (int i = 0; i < NITER; i++) {
        ...
        matmul_kernel_multithreaded(A, B, C, sA, sB, M, N, K);
        uint64_t end = timer();
