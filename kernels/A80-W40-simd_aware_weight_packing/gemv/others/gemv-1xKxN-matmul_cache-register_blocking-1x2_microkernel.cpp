...
{
    {
        ...
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            ...
            // Load A packed data (same for both j iterations)
            __m256i a_pack_int8_low = _mm256_loadu_si256((__m256i *) blockA(0, p, block_k));
            __m256i a_pack_int8_high = _mm256_loadu_si256((__m256i *) blockA(0, p + Q_BLK_SIZE, block_k));

            // Load B packed data for j and j+1
        ...
        *C(0, 0, NDIM) += hsum_float_8(sum_packed_fp_0);
        // if (j + 1 < block_n) {
            *C(0, 0+1, NDIM) += hsum_float_8(sum_packed_fp_1);
        // }
    }
}
