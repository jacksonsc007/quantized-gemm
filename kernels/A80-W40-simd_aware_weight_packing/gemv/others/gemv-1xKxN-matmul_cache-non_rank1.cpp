        ...
        for (int j = 0; j < block_n; j += 4) {
            ...
            for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE) {
                ...
                for (int ii = 0; ii < 4; ++ii) {
                        sA[0] = *blockSA(0, block_idx, block_k/Q_BLK_SIZE);
                        sA[1] = *blockSA(0, block_idx+1, block_k/Q_BLK_SIZE);
                }
                ...
                __m256i a_pack_int8[2]; // [i][0/1 for low/high block]
                        a_pack_int8[0] = _mm256_loadu_si256((__m256i *) blockA(0, p, block_k));
                        a_pack_int8[1] = _mm256_loadu_si256((__m256i *) blockA(0, p+Q_BLK_SIZE, block_k));
                ...
                // Compute all active i/j combinations
                    for (int jj = 0; jj < 4; ++jj) {
                        ...
                        __m256 tmp[2] = {
                            mul_sum_i8_pairs_float(a_pack_int8[0], b_pack_int8_low[jj]),
                            mul_sum_i8_pairs_float(a_pack_int8[1], b_pack_int8_high[jj])
                        };
            ...
            // Store results for all active i/j combinations
                ...
                for (int jj = 0; jj < 4; ++jj) {
                    ...
                    *C(0, j+jj, NDIM) += hsum_float_8(sum_packed_fp[jj]);
                }
        }
}
