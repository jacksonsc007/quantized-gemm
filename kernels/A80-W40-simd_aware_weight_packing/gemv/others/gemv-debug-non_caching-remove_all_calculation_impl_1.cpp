    ...
    const int M, const int N, const int K) 
{
    for (int j = 0; j < N; j++)
    {
        ...
        for (int p = 0; p < K; p += 2 * Q_BLK_SIZE)
        {
            ...
            volatile float sA_0 = *SA(0, block_idx, K/Q_BLK_SIZE);
            volatile float sB_0 = *SB(block_idx, j, K/Q_BLK_SIZE);
            volatile float sA_1 = *SA(0, block_idx + 1, K/Q_BLK_SIZE);
            volatile float sB_1 = *SB(block_idx + 1, j, K/Q_BLK_SIZE);
            // float fused_s_0 = 0.02;
            // float fused_s_1 = 0.02;
            // float fused_s_0 = sA_0 * sB_0;
            // float fused_s_1 = sA_1 * sB_1;
                volatile __m256i a_pack_int8_low = _mm256_loadu_si256( (__m256i *) A(0, p, K));
                volatile __m256i a_pack_int8_high = _mm256_loadu_si256( (__m256i *) A(0, p + Q_BLK_SIZE, K));
                volatile __m256i b_pack_int8 = _mm256_loadu_si256( (__m256i *) B(p/2, j, K/2));
                ...
                volatile __m256i b_pack_int8_low = _mm256_and_si256(b_pack_int8, mask);
                volatile __m256i b_pack_int8_high = _mm256_and_si256(  _mm256_srli_epi16(b_pack_int8, 4), mask);
