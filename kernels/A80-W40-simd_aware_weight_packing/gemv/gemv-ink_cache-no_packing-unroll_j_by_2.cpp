    ...
    float* C, const int block_m, const int block_n, const int block_k)
{
    ...
    for (int j = 0; j < block_n; j += 2)
    {
        ...
        for (int p = 0; p < block_k; p += 2 * Q_BLK_SIZE)
        {
            ...
            float sA_0 = *blockSA(0, block_idx, KDIM/Q_BLK_SIZE);
            float sB_0_j0 = *blockSB(block_idx, j, KDIM/Q_BLK_SIZE);
            float sB_0_j1 = *blockSB(block_idx, j+1, KDIM/Q_BLK_SIZE);
            
            float sA_1 = *blockSA(0, block_idx + 1, KDIM/Q_BLK_SIZE);
            float sB_1_j0 = *blockSB(block_idx + 1, j, KDIM/Q_BLK_SIZE);
            float sB_1_j1 = *blockSB(block_idx + 1, j+1, KDIM/Q_BLK_SIZE);
            ...
            __m256i a_pack_int8_low = _mm256_loadu_si256((__m256i *) blockA(0, p, KDIM));
            __m256i a_pack_int8_high = _mm256_loadu_si256((__m256i *) blockA(0, p + Q_BLK_SIZE, KDIM));

            // Load B packed data for j and j+1
            __m256i b_pack_int8_j0 = _mm256_loadu_si256((__m256i *) blockB(p/2, j, KDIM/2));
            __m256i b_pack_int8_j1 = _mm256_loadu_si256((__m256i *) blockB(p/2, j+1, KDIM/2));
        ...
        for (int pc = 0; pc < K; pc += KC)
        {
            int valid_KC = min(K - pc, KC);
            {
                    ...
                    matmul_kernel_naive(
                        ...
                        SB(pc / Q_BLK_SIZE, jc + jr, KDIM / Q_BLK_SIZE),
                        C(ic + ir, jc + jr, N),
                        valid_mr,
                        valid_nr,
                        valid_KC
                    ...
                    kernel(
                        ...
                        // (sA_packed + ir * valid_KC / Q_BLK_SIZE),
                        // (sB_packed + jr * valid_KC / Q_BLK_SIZE),
                        SB(pc / Q_BLK_SIZE, jc, KDIM / Q_BLK_SIZE),
                        C(0, jc, N),
                        1,
                        valid_NC,
                        valid_KC
