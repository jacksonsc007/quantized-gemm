...
)
{
    ...
    int nb_m = M / 2; // Process 2 rows at a time
    for (int i = 0; i < nb_m; ++i)
    {
        for(int j = 0; j < nb_n; j++)
        {
            struct q40_repack_2x8 * B_ptr = B_ptr_start + j * nb_k;
            struct q80_repack_1x2 * A_ptr_row0 = A_ptr_start + (i * 2) * nb_k;
            struct q80_repack_1x2 * A_ptr_row1 = A_ptr_start + (i * 2 + 1) * nb_k;
            ...
            for (int blk_id = 0; blk_id < nb_k; blk_id++)
            {
                ...
                // Process row 0
                __m256i low_blk_A_packs_row0[8];
                __m256i high_blk_A_packs_row0[8];
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A_packs_row0[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr_row0[blk_id].q_low) )[pack_idx]);
                    high_blk_A_packs_row0[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr_row0[blk_id].q_high) )[pack_idx]);
                }
                __m256i iacc_low_row0 = _mm256_setzero_si256();
                __m256i iacc_high_row0 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low_row0  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low_row0 , low_blk_B_packs  [ pack_idx ] , low_blk_A_packs_row0  [ pack_idx ]); 
                    iacc_high_row0 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high_row0, high_blk_B_packs [ pack_idx ] , high_blk_A_packs_row0 [ pack_idx ]); 
                }

                // Process row 1
                __m256i low_blk_A_packs_row1[8];
                __m256i high_blk_A_packs_row1[8];
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A_packs_row1[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr_row1[blk_id].q_low) )[pack_idx]);
                    high_blk_A_packs_row1[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr_row1[blk_id].q_high) )[pack_idx]);
                }
                __m256i iacc_low_row1 = _mm256_setzero_si256();
                __m256i iacc_high_row1 = _mm256_setzero_si256();
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low_row1  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low_row1 , low_blk_B_packs  [ pack_idx ] , low_blk_A_packs_row1  [ pack_idx ]); 
                    iacc_high_row1 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high_row1, high_blk_B_packs [ pack_idx ] , high_blk_A_packs_row1 [ pack_idx ]); 
                }
                ...
                // Process row 0 scaling
                __m256 sA_low_row0 = _mm256_set1_ps(A_ptr_row0[blk_id].s_low);
                __m256 sA_high_row0 = _mm256_set1_ps(A_ptr_row0[blk_id].s_high);
                __m256 scaled_sum_low_row0 = _mm256_set1_ps(A_ptr_row0[blk_id].scaled_sum_low);
                __m256 scaled_sum_high_row0 = _mm256_set1_ps(A_ptr_row0[blk_id].scaled_sum_high);

                __m256 s_low_row0 = _mm256_mul_ps(sA_low_row0, sB_low);
                __m256 s_high_row0 = _mm256_mul_ps(sA_high_row0, sB_high);
                acc_row0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low_row0), s_low_row0, acc_row0);
                acc_row0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high_row0), s_high_row0, acc_row0);
                acc_row0 = _mm256_fmadd_ps(minB_low, scaled_sum_low_row0, acc_row0);
                acc_row0 = _mm256_fmadd_ps(minB_high, scaled_sum_high_row0, acc_row0);

                // Process row 1 scaling
                __m256 sA_low_row1 = _mm256_set1_ps(A_ptr_row1[blk_id].s_low);
                __m256 sA_high_row1 = _mm256_set1_ps(A_ptr_row1[blk_id].s_high);
                __m256 scaled_sum_low_row1 = _mm256_set1_ps(A_ptr_row1[blk_id].scaled_sum_low);
                __m256 scaled_sum_high_row1 = _mm256_set1_ps(A_ptr_row1[blk_id].scaled_sum_high);

                __m256 s_low_row1 = _mm256_mul_ps(sA_low_row1, sB_low);
                __m256 s_high_row1 = _mm256_mul_ps(sA_high_row1, sB_high);
                acc_row1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low_row1), s_low_row1, acc_row1);
                acc_row1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high_row1), s_high_row1, acc_row1);
                acc_row1 = _mm256_fmadd_ps(minB_low, scaled_sum_low_row1, acc_row1);
                acc_row1 = _mm256_fmadd_ps(minB_high, scaled_sum_high_row1, acc_row1);
            }
            _mm256_storeu_ps(C(i * 2, j * 8, N), acc_row0);
            _mm256_storeu_ps(C(i * 2 + 1, j * 8, N), acc_row1);
        }
    }
    ...
    if (M % 2 != 0) {
        ...
        for(int j = 0; j < nb_n; j++)
        {
            ...
            _mm256_storeu_ps(C(i * 2, j * 8, N), acc_row);
        }
    }
}
