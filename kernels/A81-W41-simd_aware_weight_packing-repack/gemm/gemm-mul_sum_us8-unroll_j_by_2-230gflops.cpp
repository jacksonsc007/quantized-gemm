        ...
        for(int j = 0; j < nb_n; j += 2)
        {
            ...
            for (int blk_id = 0; blk_id < nb_k; blk_id++)
            {
                ...
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    ...
                    iacc_low1  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low1 , low_blk_B_packs_next  [ pack_idx ] , low_blk_A_packs  [ pack_idx ]); 
                    iacc_high1 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high1, high_blk_B_packs_next [ pack_idx ] , high_blk_A_packs [ pack_idx ]); 
                }
                ...
                acc_row1 = _mm256_fmadd_ps(minB_low_next, scaled_sum_low, acc_row1);
                acc_row1 = _mm256_fmadd_ps(minB_high_next, scaled_sum_high, acc_row1);
            }
            _mm256_storeu_ps(C(i, j * 8, N), acc_row0);
            _mm256_storeu_ps(C(i, (j + 1) * 8, N), acc_row1);
        }
    }
}
