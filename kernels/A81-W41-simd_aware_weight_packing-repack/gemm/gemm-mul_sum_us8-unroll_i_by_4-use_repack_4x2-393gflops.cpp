...
struct q80_repack_4x2{
    float s_low[4];
    float s_high[4];
    float scaled_sum_low[4];
    float scaled_sum_high[4];
    int8_t q_low[4][32];
    int8_t q_high[4][32];
};
...
)
{
    ...
    struct q80_repack_4x2 * A_ptr_start = (struct q80_repack_4x2 *) A_repack;
    ...
    {
        struct q80_repack_4x2 * A_ptr = A_ptr_start + i * nb_k;
        for(int j = 0; j < nb_n; j++)
        {
            ...
            for (int blk_id = 0; blk_id < nb_k; blk_id++)
            {
                ...
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    low_blk_A_packs[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr[blk_id].q_low[0]) )[pack_idx]);
                    high_blk_A_packs[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr[blk_id].q_high[0]) )[pack_idx]);
                    low_blk_A_packs_next[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr[blk_id].q_low[1]) )[pack_idx]);
                    high_blk_A_packs_next[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr[blk_id].q_high[1]) )[pack_idx]);
                    low_blk_A_packs_next2[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr[blk_id].q_low[2]) )[pack_idx]);
                    high_blk_A_packs_next2[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr[blk_id].q_high[2]) )[pack_idx]);
                    low_blk_A_packs_next3[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr[blk_id].q_low[3]) )[pack_idx]);
                    high_blk_A_packs_next3[pack_idx] = _mm256_set1_epi32(( (int32_t *)(A_ptr[blk_id].q_high[3]) )[pack_idx]);
                }
                ...
                __m256 sA_low = _mm256_set1_ps(A_ptr[blk_id].s_low[0]);
                __m256 sA_high = _mm256_set1_ps(A_ptr[blk_id].s_high[0]);
                __m256 scaled_sum_low = _mm256_set1_ps(A_ptr[blk_id].scaled_sum_low[0]);
                __m256 scaled_sum_high = _mm256_set1_ps(A_ptr[blk_id].scaled_sum_high[0]);

                __m256 sA_low_next = _mm256_set1_ps(A_ptr[blk_id].s_low[1]);
                __m256 sA_high_next = _mm256_set1_ps(A_ptr[blk_id].s_high[1]);
                __m256 scaled_sum_low_next = _mm256_set1_ps(A_ptr[blk_id].scaled_sum_low[1]);
                __m256 scaled_sum_high_next = _mm256_set1_ps(A_ptr[blk_id].scaled_sum_high[1]);

                __m256 sA_low_next2 = _mm256_set1_ps(A_ptr[blk_id].s_low[2]);
                __m256 sA_high_next2 = _mm256_set1_ps(A_ptr[blk_id].s_high[2]);
                __m256 scaled_sum_low_next2 = _mm256_set1_ps(A_ptr[blk_id].scaled_sum_low[2]);
                __m256 scaled_sum_high_next2 = _mm256_set1_ps(A_ptr[blk_id].scaled_sum_high[2]);

                __m256 sA_low_next3 = _mm256_set1_ps(A_ptr[blk_id].s_low[3]);
                __m256 sA_high_next3 = _mm256_set1_ps(A_ptr[blk_id].s_high[3]);
                __m256 scaled_sum_low_next3 = _mm256_set1_ps(A_ptr[blk_id].scaled_sum_low[3]);
                __m256 scaled_sum_high_next3 = _mm256_set1_ps(A_ptr[blk_id].scaled_sum_high[3]);
            ...
            _mm256_storeu_ps(C(i * 4, j * 8, N), acc_row0);
            _mm256_storeu_ps(C(i * 4 + 1, j * 8, N), acc_row1);
            _mm256_storeu_ps(C(i * 4 + 2, j * 8, N), acc_row2);
            _mm256_storeu_ps(C(i * 4 + 3, j * 8, N), acc_row3);
        }
    }
}
...
int main() {
    ...
    */
    ...
    // repack A in row-major order. Pack size (4 x 64)
    int num_repack_blk_A_along_K = K / (2 * Q_BLK_SIZE);
    struct q80_repack_4x2 * A_repack_start = (struct q80_repack_4x2 *) A_repack;
    for (int i = 0; i < M; i += 4) 
    {
        struct q80_repack_4x2 * A_ptr = A_repack_start + (i / 4) * num_repack_blk_A_along_K;
        for (int blk_id = 0; blk_id < num_repack_blk_A_along_K; blk_id++) 
        {
            for (int row = 0; row < 4; row++) {
                float s_low =  *SA(i + row, blk_id * 2, K / Q_BLK_SIZE);
                float s_high = *SA(i + row, blk_id * 2 + 1, K / Q_BLK_SIZE);
                float scaled_sum_low = *ScaledSumA(i + row, blk_id * 2, K / Q_BLK_SIZE);
                float scaled_sum_high = *ScaledSumA(i + row, blk_id * 2 + 1, K / Q_BLK_SIZE);
                A_ptr[blk_id].s_low[row] = s_low;
                A_ptr[blk_id].s_high[row] = s_high;
                A_ptr[blk_id].scaled_sum_low[row] = scaled_sum_low;
                A_ptr[blk_id].scaled_sum_high[row] = scaled_sum_high;
                for (int q = 0; q < Q_BLK_SIZE; q++) 
                {
                    A_ptr[blk_id].q_low[row][q] =  *A(i + row, blk_id * 2 * Q_BLK_SIZE + q, K);
                    A_ptr[blk_id].q_high[row][q] = *A(i + row, blk_id * 2 * Q_BLK_SIZE + q + Q_BLK_SIZE, K);
                }
            }
        }
    }
    ...
    for (int i = 0; i < M; i += 4) 
    {
        ...
        {
            struct q80_repack_4x2 * A_ptr = A_repack_start + (i / 4) * num_repack_blk_A_along_K + j;
            for (int row = 0; row < 4; row++) {
                // comparea with original value
                for (int q = 0; q < Q_BLK_SIZE; q++) 
                {
                    int8_t a_low = A_ptr->q_low[row][q];
                    int8_t a_high = A_ptr->q_high[row][q];
                    int8_t a_low_orig = *A(i + row, j * 2 * Q_BLK_SIZE + q, K);
                    int8_t a_high_orig = *A(i + row, j * 2 * Q_BLK_SIZE + q + Q_BLK_SIZE, K);
                    if (a_low != a_low_orig || a_high != a_high_orig) 
                    {
                        printf("\e[31m[ERROR]\e[m Repack A[%d][%d] mismatch: %d vs %d, %d vs %d\n", i + row, j, a_low, a_low_orig, a_high, a_high_orig);
                        return 1;
                    }
                }
                // compare scaling factors
                float s_low = A_ptr->s_low[row];
                float s_high = A_ptr->s_high[row];
                float s_low_orig = *SA(i + row, j * 2, K / Q_BLK_SIZE);
                float s_high_orig = *SA(i + row, j * 2 + 1, K / Q_BLK_SIZE);  
                if (s_low != s_low_orig || s_high != s_high_orig) 
                {
                    printf("\e[31m[ERROR]\e[m Repack A[%d][%d] scaling factors mismatch: %f vs %f, %f vs %f\n", i + row, j, s_low, s_low_orig, s_high, s_high_orig);
                    return 1;
                }
                
                // compare scaled sum
                float scaled_sum_low = A_ptr->scaled_sum_low[row];
                float scaled_sum_high = A_ptr->scaled_sum_high[row];
                float scaled_sum_low_orig = *ScaledSumA(i + row, j * 2, K / Q_BLK_SIZE);
                float scaled_sum_high_orig = *ScaledSumA(i + row, j * 2 + 1, K / Q_BLK_SIZE);
                if (scaled_sum_low != scaled_sum_low_orig || scaled_sum_high != scaled_sum_high_orig) 
                {
                    printf("\e[31m[ERROR]\e[m Repack A[%d][%d] scaled sum mismatch: %f vs %f, %f vs %f\n", i + row, j, scaled_sum_low, scaled_sum_low_orig, scaled_sum_high, scaled_sum_high_orig);
                    return 1;
                }
            }
        }
