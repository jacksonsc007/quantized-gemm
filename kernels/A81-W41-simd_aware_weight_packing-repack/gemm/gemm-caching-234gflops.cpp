...
)
{
    ...
    const int mc = MC;  // Block size for M dimension
    const int nc = NC;  // Block size for N dimension
    const int kc = KC;  // Block size for K dimension
    
    // Number of blocks in each dimension
    const int nb_k = K / (2 * Q_BLK_SIZE);
    const int nb_n = N / 8;
    const int nb_m = M;
    
    // Process blocks of size mc x nc
    for (int i = 0; i < nb_m; i += mc) {
        const int m_end = min(i + mc, nb_m);
        
        for (int j = 0; j < nb_n; j += nc) {
            const int n_end = min(j + nc, nb_n);
            
            // Process each block
            for (int ii = i; ii < m_end; ++ii) {
                for (int jj = j; jj < n_end; ++jj) {
                    struct q40_repack_2x8 * B_ptr = B_ptr_start + jj * nb_k;
                    struct q80_repack_1x2 * A_ptr = A_ptr_start + ii * nb_k;
                    __m256 acc_row = _mm256_setzero_ps();
                    
                    // Process K dimension in blocks of size kc
                    for (int k = 0; k < nb_k; k += kc) {
                        const int k_end = min(k + kc, nb_k);
                        
                        for (int blk_id = k; blk_id < k_end; blk_id++) {
                            struct q40_repack_2x8 B_block = B_ptr[blk_id];
                            
                            // Process low and high blocks
                            ...
                            for (int pack_idx = 0; pack_idx < 8; pack_idx++) {
                                ...
                                low_blk_B_packs[pack_idx] = _mm256_and_si256(B_coupled_pack, _mm256_set1_epi8(0x0f));
                                high_blk_B_packs[pack_idx] = _mm256_and_si256(_mm256_srli_epi16(B_coupled_pack, 4), _mm256_set1_epi8(0x0f));
                            }
                    ...
                    _mm256_storeu_ps(C(ii, jj * 8, N), acc_row);
                }
            }
        }
    }
