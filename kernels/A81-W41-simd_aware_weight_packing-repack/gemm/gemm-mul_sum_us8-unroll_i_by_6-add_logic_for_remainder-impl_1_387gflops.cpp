...
)
{
    ...
    for (int i = 0; i < nb_m; i += 6)
    {
        ...
        int remaining_rows = min(6, nb_m - i);
        if (remaining_rows < 6) {
            // Process remaining rows one by one
            for (int r = 0; r < remaining_rows; r++) {
                struct q80_repack_1x2 * A_ptr = A_ptr_start + (i + r) * nb_k;
                for(int j = 0; j < nb_n; j++)
                {
                    ...
                    _mm256_storeu_ps(C(i + r, j * 8, N), acc_row);
                }
            }
            continue;
        }

        // Original code for processing 6 rows at once
