...
void matmul_repack_per_thread(
    void * A_repack,
    void * B_repack,
    float* C,
    const int M, const int N, const int K, const int start_col, const int end_col
)
{
    ...
    const int M, const int N, const int K)
{
    ...
    #pragma omp parallel num_threads(NTHREADS)
    {
        ...
        // Distribute column blocks (j) using dynamic scheduling
        int total_blocks_n = N / 8 + ((N % 8) ? 1 : 0); // Total 8-wide column blocks in B
        #pragma omp for schedule(dynamic)
        for (int block_j = 0; block_j < total_blocks_n; ++block_j)
        {
            int start_col = block_j * 8;
            int end_col = (start_col + 8) > N ? N : (start_col + 8);

            // Call per-thread kernel on assigned block
            matmul_repack_per_thread(A_repack, B_repack, C, M, N, K, start_col, end_col);
        }
    }
}
