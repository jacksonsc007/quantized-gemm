/*
Based on v4, omit the j loop in the kernel_per_thread, due to the task assignment logic.
*/
#include <assert.h>
#include <immintrin.h>
#include <math.h>
    ...
    const int M, const int N, const int K)
{
    #pragma omp parallel num_threads(NTHREADS)
    {
        ...
        // Each iteration will process 8 columns (since j is in steps of 8)
        #pragma omp for schedule(dynamic)
        for (int j = 0; j < N; j += 8) {
            int end_col = min(j + 8, N);
            matmul_repack_per_thread(A_repack, B_repack, C, M, N, K, j, end_col);
        }
    }
}
