...
)
{
    ...
    for (int i = 0; i < nb_m; i += 4)
    {
        ...
        struct q80_repack_1x2 * A_ptr_next = A_ptr_start + (i + 1) * nb_k;
        struct q80_repack_1x2 * A_ptr_next2 = A_ptr_start + (i + 2) * nb_k;
        struct q80_repack_1x2 * A_ptr_next3 = A_ptr_start + (i + 3) * nb_k;
        
        for(int j = 0; j < nb_n; j += 2)
        {
            struct q40_repack_2x8 * B_ptr = B_ptr_start + j * nb_k;
            struct q40_repack_2x8 * B_ptr_next = B_ptr_start + (j + 1) * nb_k;
            
            __m256 acc_row0_0 = _mm256_setzero_ps();
            __m256 acc_row0_1 = _mm256_setzero_ps();
            __m256 acc_row1_0 = _mm256_setzero_ps();
            __m256 acc_row1_1 = _mm256_setzero_ps();
            __m256 acc_row2_0 = _mm256_setzero_ps();
            __m256 acc_row2_1 = _mm256_setzero_ps();
            __m256 acc_row3_0 = _mm256_setzero_ps();
            __m256 acc_row3_1 = _mm256_setzero_ps();
            
            for (int blk_id = 0; blk_id < nb_k; blk_id++)
            {
                struct q40_repack_2x8 B_block =  B_ptr[blk_id];
                struct q40_repack_2x8 B_block_next =  B_ptr_next[blk_id];
                // each pack contains 4 elements in the quantization block
                __m256i low_blk_B_packs[8];
                __m256i high_blk_B_packs[8];
                __m256i low_blk_B_packs_next[8];
                __m256i high_blk_B_packs_next[8];
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    __m256i B_coupled_pack = _mm256_loadu_si256((__m256i *) (B_block.q_coupled + pack_idx * 32));
                    __m256i B_coupled_pack_next = _mm256_loadu_si256((__m256i *) (B_block_next.q_coupled + pack_idx * 32));
                    ...
                    __m256i low_blk_B_pack_next = _mm256_and_si256(B_coupled_pack_next, _mm256_set1_epi8(0x0f));
                    __m256i high_blk_B_pack_next = _mm256_and_si256(_mm256_srli_epi16(B_coupled_pack_next, 4), _mm256_set1_epi8(0x0f));
                    low_blk_B_packs[pack_idx] = low_blk_B_pack;
                    high_blk_B_packs[pack_idx] = high_blk_B_pack;
                    low_blk_B_packs_next[pack_idx] = low_blk_B_pack_next;
                    high_blk_B_packs_next[pack_idx] = high_blk_B_pack_next;
                }
                
                // Process four rows of A simultaneously
                ...
                __m256i iacc_low0_0 = _mm256_setzero_si256();
                __m256i iacc_high0_0 = _mm256_setzero_si256();
                __m256i iacc_low0_1 = _mm256_setzero_si256();
                __m256i iacc_high0_1 = _mm256_setzero_si256();
                __m256i iacc_low1_0 = _mm256_setzero_si256();
                __m256i iacc_high1_0 = _mm256_setzero_si256();
                __m256i iacc_low1_1 = _mm256_setzero_si256();
                __m256i iacc_high1_1 = _mm256_setzero_si256();
                __m256i iacc_low2_0 = _mm256_setzero_si256();
                __m256i iacc_high2_0 = _mm256_setzero_si256();
                __m256i iacc_low2_1 = _mm256_setzero_si256();
                __m256i iacc_high2_1 = _mm256_setzero_si256();
                __m256i iacc_low3_0 = _mm256_setzero_si256();
                __m256i iacc_high3_0 = _mm256_setzero_si256();
                __m256i iacc_low3_1 = _mm256_setzero_si256();
                __m256i iacc_high3_1 = _mm256_setzero_si256();
                
                for (int pack_idx = 0; pack_idx < 8; pack_idx+=1)
                {
                    iacc_low0_0  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low0_0 , low_blk_B_packs  [ pack_idx ] , low_blk_A_packs  [ pack_idx ]); 
                    iacc_high0_0 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high0_0, high_blk_B_packs [ pack_idx ] , high_blk_A_packs [ pack_idx ]); 
                    iacc_low0_1  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low0_1 , low_blk_B_packs_next  [ pack_idx ] , low_blk_A_packs  [ pack_idx ]); 
                    iacc_high0_1 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high0_1, high_blk_B_packs_next [ pack_idx ] , high_blk_A_packs [ pack_idx ]); 
                    iacc_low1_0  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low1_0 , low_blk_B_packs  [ pack_idx ] , low_blk_A_packs_next  [ pack_idx ]); 
                    iacc_high1_0 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high1_0, high_blk_B_packs [ pack_idx ] , high_blk_A_packs_next [ pack_idx ]); 
                    iacc_low1_1  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low1_1 , low_blk_B_packs_next  [ pack_idx ] , low_blk_A_packs_next  [ pack_idx ]); 
                    iacc_high1_1 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high1_1, high_blk_B_packs_next [ pack_idx ] , high_blk_A_packs_next [ pack_idx ]); 
                    iacc_low2_0  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low2_0 , low_blk_B_packs  [ pack_idx ] , low_blk_A_packs_next2  [ pack_idx ]); 
                    iacc_high2_0 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high2_0, high_blk_B_packs [ pack_idx ] , high_blk_A_packs_next2 [ pack_idx ]); 
                    iacc_low2_1  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low2_1 , low_blk_B_packs_next  [ pack_idx ] , low_blk_A_packs_next2  [ pack_idx ]); 
                    iacc_high2_1 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high2_1, high_blk_B_packs_next [ pack_idx ] , high_blk_A_packs_next2 [ pack_idx ]); 
                    iacc_low3_0  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low3_0 , low_blk_B_packs  [ pack_idx ] , low_blk_A_packs_next3  [ pack_idx ]); 
                    iacc_high3_0 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high3_0, high_blk_B_packs [ pack_idx ] , high_blk_A_packs_next3 [ pack_idx ]); 
                    iacc_low3_1  = mul_sum_us8_pairs_acc_int32x8 ( iacc_low3_1 , low_blk_B_packs_next  [ pack_idx ] , low_blk_A_packs_next3  [ pack_idx ]); 
                    iacc_high3_1 = mul_sum_us8_pairs_acc_int32x8 ( iacc_high3_1, high_blk_B_packs_next [ pack_idx ] , high_blk_A_packs_next3 [ pack_idx ]); 
                }
                ...
                __m256 sB_low_next = _mm256_loadu_ps(B_block_next.s_low);
                __m256 sB_high_next = _mm256_loadu_ps(B_block_next.s_high);
                __m256 minB_low_next = _mm256_loadu_ps(B_block_next.min_low);
                __m256 minB_high_next = _mm256_loadu_ps(B_block_next.min_high);
                ...
                __m256 s_low_next = _mm256_mul_ps(sA_low, sB_low_next);
                __m256 s_high_next = _mm256_mul_ps(sA_high, sB_high_next);

                __m256 s_low_next_row = _mm256_mul_ps(sA_low_next, sB_low);
                __m256 s_high_next_row = _mm256_mul_ps(sA_high_next, sB_high);
                __m256 s_low_next_row_next = _mm256_mul_ps(sA_low_next, sB_low_next);
                __m256 s_high_next_row_next = _mm256_mul_ps(sA_high_next, sB_high_next);

                __m256 s_low_next_row2 = _mm256_mul_ps(sA_low_next2, sB_low);
                __m256 s_high_next_row2 = _mm256_mul_ps(sA_high_next2, sB_high);
                __m256 s_low_next_row2_next = _mm256_mul_ps(sA_low_next2, sB_low_next);
                __m256 s_high_next_row2_next = _mm256_mul_ps(sA_high_next2, sB_high_next);

                __m256 s_low_next_row3 = _mm256_mul_ps(sA_low_next3, sB_low);
                __m256 s_high_next_row3 = _mm256_mul_ps(sA_high_next3, sB_high);
                __m256 s_low_next_row3_next = _mm256_mul_ps(sA_low_next3, sB_low_next);
                __m256 s_high_next_row3_next = _mm256_mul_ps(sA_high_next3, sB_high_next);

                // First row, first column
                acc_row0_0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low0_0), s_low, acc_row0_0);
                acc_row0_0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high0_0), s_high, acc_row0_0);
                acc_row0_0 = _mm256_fmadd_ps(minB_low, scaled_sum_low, acc_row0_0);
                acc_row0_0 = _mm256_fmadd_ps(minB_high, scaled_sum_high, acc_row0_0);

                // First row, second column
                acc_row0_1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low0_1), s_low_next, acc_row0_1);
                acc_row0_1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high0_1), s_high_next, acc_row0_1);
                acc_row0_1 = _mm256_fmadd_ps(minB_low_next, scaled_sum_low, acc_row0_1);
                acc_row0_1 = _mm256_fmadd_ps(minB_high_next, scaled_sum_high, acc_row0_1);

                // Second row, first column
                acc_row1_0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low1_0), s_low_next_row, acc_row1_0);
                acc_row1_0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high1_0), s_high_next_row, acc_row1_0);
                acc_row1_0 = _mm256_fmadd_ps(minB_low, scaled_sum_low_next, acc_row1_0);
                acc_row1_0 = _mm256_fmadd_ps(minB_high, scaled_sum_high_next, acc_row1_0);

                // Second row, second column
                acc_row1_1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low1_1), s_low_next_row_next, acc_row1_1);
                acc_row1_1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high1_1), s_high_next_row_next, acc_row1_1);
                acc_row1_1 = _mm256_fmadd_ps(minB_low_next, scaled_sum_low_next, acc_row1_1);
                acc_row1_1 = _mm256_fmadd_ps(minB_high_next, scaled_sum_high_next, acc_row1_1);

                // Third row, first column
                acc_row2_0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low2_0), s_low_next_row2, acc_row2_0);
                acc_row2_0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high2_0), s_high_next_row2, acc_row2_0);
                acc_row2_0 = _mm256_fmadd_ps(minB_low, scaled_sum_low_next2, acc_row2_0);
                acc_row2_0 = _mm256_fmadd_ps(minB_high, scaled_sum_high_next2, acc_row2_0);

                // Third row, second column
                acc_row2_1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low2_1), s_low_next_row2_next, acc_row2_1);
                acc_row2_1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high2_1), s_high_next_row2_next, acc_row2_1);
                acc_row2_1 = _mm256_fmadd_ps(minB_low_next, scaled_sum_low_next2, acc_row2_1);
                acc_row2_1 = _mm256_fmadd_ps(minB_high_next, scaled_sum_high_next2, acc_row2_1);

                // Fourth row, first column
                acc_row3_0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low3_0), s_low_next_row3, acc_row3_0);
                acc_row3_0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high3_0), s_high_next_row3, acc_row3_0);
                acc_row3_0 = _mm256_fmadd_ps(minB_low, scaled_sum_low_next3, acc_row3_0);
                acc_row3_0 = _mm256_fmadd_ps(minB_high, scaled_sum_high_next3, acc_row3_0);

                // Fourth row, second column
                acc_row3_1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_low3_1), s_low_next_row3_next, acc_row3_1);
                acc_row3_1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(iacc_high3_1), s_high_next_row3_next, acc_row3_1);
                acc_row3_1 = _mm256_fmadd_ps(minB_low_next, scaled_sum_low_next3, acc_row3_1);
                acc_row3_1 = _mm256_fmadd_ps(minB_high_next, scaled_sum_high_next3, acc_row3_1);
            }
            _mm256_storeu_ps(C(i, j * 8, N), acc_row0_0);
            _mm256_storeu_ps(C(i, (j + 1) * 8, N), acc_row0_1);
            _mm256_storeu_ps(C(i + 1, j * 8, N), acc_row1_0);
            _mm256_storeu_ps(C(i + 1, (j + 1) * 8, N), acc_row1_1);
            _mm256_storeu_ps(C(i + 2, j * 8, N), acc_row2_0);
            _mm256_storeu_ps(C(i + 2, (j + 1) * 8, N), acc_row2_1);
            _mm256_storeu_ps(C(i + 3, j * 8, N), acc_row3_0);
            _mm256_storeu_ps(C(i + 3, (j + 1) * 8, N), acc_row3_1);
        }
    }
}
